...
impl AppContext {
    ...
    #[cfg(test)]
    fn from_owned(github: GitHubContext, stack: Vec<ResolvedChange>) -> Self {
        Self::new(
            github,
            StackResolution {
                owned: stack,
...
impl CacheStore {
    #[cfg(test)]
    fn load_current(runner: &impl CommandRunner) -> Result<Self> {
        let repo_dir = resolve_current_jj_repo_dir(runner)?;
        Self::load(repo_dir.join(CONFIG_PREFIX).join("cache.sqlite"))
    }
                ...
                match fetch_pr_for_merge(runner, &context.github, &change.change_id, pr_number) {
                    Ok(pr) => {
                            ...
                            validate_merge_frozen_dependencies(runner, config, &context, &pr)
                        {
                            ...
                            if let Err(error) = validate_pr_ready_for_merge(
                                config,
                                change,
                                &entry,
                                &pr,
...
/// - *Threaded/parallel jj pushes*: `CommandRunner` is synchronous, `FakeRunner`
///   is `!Send`, and concurrent `jj` contends on the op-log lock. Rejected in
///   favor of a single batched `jj git push --bookmark …`.
...
fn merge_stack(
    ...
    for (index, change) in context.stack.iter().enumerate() {
        let (entry, mut pr) =
            match resolve_merge_pr(runner, config, &context, change, diagnostics) {
    ...
    summary.cleaned_branches +=
        cleanup_merged_branches(runner, config, &context.github, &merged_branches, diagnostics);
...
fn fast_forward_trunk_over_stack(
    ...
    let is_ancestor =
        runner.run("git", &["merge-base", "--is-ancestor", remote.as_str(), top_commit])?;
...
#[cfg(test)]
fn stack_comment_body(
    context: &AppContext,
    entries: &[(String, PrCacheEntry)],
    current_change_id: &str,
    trunk: &str,
) -> String {
    stack_comment_body_with_frozen(context, &[], entries, current_change_id, trunk)
}
...
fn lookup_pr(
    ...
    if let Some(entry) = store.get_pr(&github.repo, change_id) {
        if let Some(entry) = lookup_cached_pr(runner, github, change_id, head_branch, entry)
            .map(Some)
            .unwrap_or(None)
        {
    ...
    lookup_open_pr_by_head_branch(runner, github, change_id, head_branch)
}
...
mod tests {
    ...
    #[test]
    fn exposes_required_subcommands_with_global_verbose() {
        for command in ["submit", "sync", "merge", "status"] {
            let cli = Cli::try_parse_from(["jj-stack", "--verbose", command]);
            assert!(cli.is_ok(), "{command} should accept global --verbose");
            assert!(cli.unwrap_or_else(|error| panic!("{error}")).verbose);
        }
        let cli = Cli::try_parse_from(["jj-stack", "--verbose", "get", "1"]);
        assert!(cli.is_ok(), "get should accept global --verbose");
        assert!(cli.unwrap_or_else(|error| panic!("{error}")).verbose);
    }

    #[test]
    fn subcommands_default_to_stack_revset() {
        for command in ["submit", "sync", "merge", "status"] {
            let cli = Cli::try_parse_from(["jj-stack", command])
                .unwrap_or_else(|error| panic!("{error}"));
            let revset = match cli.command {
                Commands::Submit(options) => options.revset,
                Commands::Sync(options) => options.stack.revset,
                Commands::Merge(options) => options.stack.revset,
                Commands::Status(options) => options.stack.revset,
                Commands::Get(_) | Commands::Unfreeze(_) | Commands::Pr(_) => {
                    unreachable!("target commands do not use a revset")
                }
            };

            assert_eq!(revset, DEFAULT_STACK_REVSET);
        }
    }

    #[test]
    fn get_accepts_pr_number_or_github_url() -> Result<()> {
        assert_eq!(
            parse_get_target("5137", "owner/repo")?,
            GetTarget::PullRequest {
        ...
        assert_eq!(
            parse_get_target("https://github.com/rivet-dev/rivet/pull/5137", "owner/repo")?,
            GetTarget::PullRequest {
                repo: "rivet-dev/rivet".to_owned(),
                number: 5137
            }
        );
        assert_eq!(
            parse_get_target("refs/heads/stack/fix-types-pmtxtmlu", "owner/repo")?,
            GetTarget::BranchOrChange {
                repo: "owner/repo".to_owned(),
                value: "stack/fix-types-pmtxtmlu".to_owned()
            }
        );
        Ok(())
    }
    ...
    #[test]
    fn get_resolves_pr_number_and_url_targets() -> Result<()> {
        for target in ["12", "https://github.com/owner/repo/pull/12"] {
            let runner = FakeRunner::new([fake_pr_fetch(
                12,
                &pr_json_with_details(
                    12,
                    "OPEN",
                    "stack/fix-types-pmtxtmlu",
                    DEFAULT_TRUNK,
                    "headsha",
                    "basesha",
                    "fix types",
            ...
            let pr = resolve_get_target_pr(
                &runner,
                &test_github(),
                parse_get_target(target, "owner/repo")?,
            )?;

            assert_eq!(pr.number, 12);
            assert_eq!(pr.head_ref_name, "stack/fix-types-pmtxtmlu");
        }
        Ok(())
    }

    #[test]
    fn pr_resolves_url_for_pr_number_without_network() -> Result<()> {
        let runner = FakeRunner::new([]);
        let (number, url) = resolve_pr_url(&runner, &test_github(), Some("42"))?;
        assert_eq!(number, 42);
        assert_eq!(url, "https://github.com/owner/repo/pull/42");
        assert!(
            runner.calls().is_empty(),
            "a bare PR number should not require any commands"
        );
        Ok(())
    }

    #[test]
    fn pr_resolves_url_from_pr_url_target() -> Result<()> {
        let runner = FakeRunner::new([]);
        let (number, url) = resolve_pr_url(
            &runner,
            &test_github(),
            Some("https://github.com/rivet-dev/rivet/pull/5137"),
        )?;
        assert_eq!(number, 5137);
        assert_eq!(url, "https://github.com/rivet-dev/rivet/pull/5137");
        Ok(())
    }

    #[test]
    fn pr_resolves_url_from_branch_target() -> Result<()> {
        let runner = FakeRunner::new([fake_success(
            "gh",
            &[
                "pr",
                "list",
        ...
        let (number, url) =
            resolve_pr_url(&runner, &test_github(), Some("stack/fix-types-pmtxtmlu"))?;
        assert_eq!(number, 7);
    ...
    #[test]
    fn current_change_id_reads_working_copy() -> Result<()> {
        let runner = FakeRunner::new([fake_success(
            "jj",
            &["log", "--no-graph", "-r", "@", "-T", "change_id ++ \"\\n\""],
            "pmtxtmluabcdefgh\n",
        )]);
        assert_eq!(current_change_id(&runner)?, "pmtxtmluabcdefgh");
        Ok(())
    }

    #[test]
    fn open_url_invokes_platform_opener() -> Result<()> {
        ...
        let url = "https://github.com/owner/repo/pull/9";
        let args: Vec<&str> = if cfg!(target_os = "windows") {
            vec!["/C", "start", "", url]
        } else {
            vec![url]
        };
        let runner = FakeRunner::new([fake_success(program, &args, "")]);
        open_url(&runner, url)?;
        assert!(runner.calls().contains(&command_key(program, &args)));
        Ok(())
    }

    #[test]
    fn get_resolves_branch_name_target() -> Result<()> {
        let runner = FakeRunner::new([fake_success(
            "gh",
            &[
                "pr",
        ...
        let pr = resolve_get_target_pr(
            &runner,
            &test_github(),
            parse_get_target("stack/fix-types-pmtxtmlu", "owner/repo")?,
        )?;
    ...
    #[test]
    fn get_resolves_change_id_prefix_target() -> Result<()> {
        let runner = FakeRunner::new([fake_success(
            "gh",
            &[
                "pr",
        ...
        let pr = resolve_get_target_pr(
            &runner,
            &test_github(),
            parse_get_target("pmtxtmluxqzxvksysypyzxqumkuvvlyo", "owner/repo")?,
        )?;
    ...
    #[test]
    fn unfreeze_adopts_frozen_pr_branch_and_writes_cache_hint() -> Result<()> {
        let workspace = test_workspace("unfreeze-success")?;
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/adopt-title-changeon";
        let runner = FakeRunner::new([
            fake_success(
                "gh",
                &[
            ...
            fake_success("gh", &["api", "user", "--jq", ".login"], "octocat"),
            fake_success(
                "gh",
            ...
            fake_success(
                "jj",
                &["bookmark", "track", "--remote", DEFAULT_REMOTE, branch],
                "",
            ),
            fake_success("jj", &["bookmark", "delete", "jj-stack/frozen/pr-11"], ""),
            fake_success(
                "jj",
                &["log", "--no-graph", "-r", commit, "-T", "immutable"],
                "false",
            ),
            fake_success(
                "jj",
                ...
                workspace
                    .to_str()
                    .unwrap_or_else(|| panic!("utf-8 workspace")),
            ),
            fake_success(
                "jj",
                &[
            ...
            fake_success("git", &["show", "-s", "--format=%T", commit], "tree"),
        ]);
        let cli = Cli::try_parse_from(["jj-stack", "unfreeze", "11"])?;
        ...
        assert!(
            calls
                .iter()
                .any(|call| call == "jj bookmark track --remote origin stack/adopt-title-changeon"),
            "unfreeze should track the PR branch: {calls:?}"
        );
        assert!(
            calls
                .iter()
                .any(|call| call == "jj bookmark delete jj-stack/frozen/pr-11"),
            "unfreeze should remove the frozen bookmark: {calls:?}"
        );
        let store = CacheStore::load_current(&runner)?;
        assert_eq!(
            store
                .get_pr("owner/repo", "changeone")
    ...
    #[test]
    fn unfreeze_accepts_each_target_form() -> Result<()> {
        let commit = "1111111111111111111111111111111111111111";
        let base = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let branch = "stack/adopt-title-changeon";
        let pr = pr_json_with_details(
            11,
            "OPEN",
            branch,
            DEFAULT_TRUNK,
        ...
        for (target, target_lookup) in [
            ("11", fake_pr_fetch(11, &pr)),
            (
            ...
            let cli = Cli::try_parse_from(["jj-stack", "--dry-run", "unfreeze", target])?;

            run(cli, &runner).with_context(|| format!("unfreeze target `{target}`"))?;

            let calls = runner.calls();
            assert!(
                calls.iter().any(|call| {
                    call == "jj log --no-graph -r jj-stack/frozen/pr-11 -T commit_id"
                }),
                "unfreeze should validate the frozen bookmark for `{target}`: {calls:?}"
            );
            assert!(
                calls
                    .iter()
                    .all(|call| !call.starts_with("jj bookmark track")),
                "dry-run unfreeze should not track bookmarks for `{target}`: {calls:?}"
            );
            assert!(
                calls
                    .iter()
                    .all(|call| !call.starts_with("jj bookmark delete")),
                "dry-run unfreeze should not delete bookmarks for `{target}`: {calls:?}"
            );
        }
    ...
    #[test]
    fn unfreeze_fails_when_pr_advanced_remotely() {
        let old_commit = "1111111111111111111111111111111111111111";
        let new_commit = "2222222222222222222222222222222222222222";
        let branch = "stack/adopt-title-changeon";
        let runner = FakeRunner::new([
            fake_success(
                "gh",
        ...
        let error = run(cli, &runner)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("advanced PR should fail"));

        assert!(error.contains("run `jj-stack sync` first"), "{error}");
        let calls = runner.calls();
        assert!(
            calls
                .iter()
                .all(|call| !call.starts_with("jj bookmark delete")),
            "unfreeze should not delete frozen bookmark when PR advanced: {calls:?}"
        );
    }

    #[test]
    fn unfreeze_fails_without_repo_push_permission() {
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/adopt-title-changeon";
        let runner = FakeRunner::new([
            fake_success(
        ...
        let error = run(cli, &runner)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("missing push permission should fail"));

        assert!(error.contains("does not have push permission"), "{error}");
        let calls = runner.calls();
        assert!(
            calls.iter().all(|call| !call.starts_with("jj bookmark")),
            "unfreeze should not mutate jj bookmarks without push permission: {calls:?}"
        );
    }

    #[test]
    fn unfreeze_fails_when_revision_remains_immutable() {
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/adopt-title-changeon";
        let runner = FakeRunner::new([
            fake_success(
        ...
        let error = run(cli, &runner)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("remaining immutability should fail"));

        assert!(error.contains("still immutable"), "{error}");
        assert!(
            error.contains("another frozen bookmark, tag, trunk"),
            "{error}"
        );
    }

    #[test]
    fn get_rejects_ambiguous_change_id_prefix_with_candidates() {
        let runner = FakeRunner::new([fake_success(
            "gh",
            &[
                "pr",
        ...
        let error = resolve_get_target_pr(
            &runner,
            &test_github(),
            parse_get_target("pmtxtmluxqzxvksysypyzxqumkuvvlyo", "owner/repo")
                .unwrap_or_else(|error| panic!("{error}")),
        ...
        .err()
        .map(|error| error.to_string())
        .unwrap_or_else(|| panic!("ambiguous change prefix should fail"));

        assert!(error.contains("matched multiple open PRs"), "{error}");
        assert!(error.contains("#12"), "{error}");
        assert!(error.contains("#13"), "{error}");
    }

    #[test]
    fn get_validation_rejects_topology_mismatch() {
        let bottom: GhPr = serde_json::from_str(&pr_json_with_details(
            11,
            "OPEN",
        ...
        .unwrap_or_else(|error| panic!("parse PR: {error}"));
        let top: GhPr = serde_json::from_str(&pr_json_with_details(
            12,
            "OPEN",
        ...
        let error = validate_get_pr_stack(&test_config(), &test_github(), 12, &[bottom, top])
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("topology mismatch should fail"));

        assert!(error.contains("stack topology mismatch"), "{error}");
        assert!(error.contains("PR #12"), "{error}");
    }

    #[test]
    fn get_validation_rejects_fork_backed_prs() {
        let mut value: serde_json::Value = serde_json::from_str(&pr_json_with_details(
            11,
            "OPEN",
            "stack/bottom-title-bottomch",
            DEFAULT_TRUNK,
        ...
        .unwrap_or_else(|error| panic!("parse PR json: {error}"));
        value["headRepository"]["nameWithOwner"] = serde_json::json!("fork/repo");
        let pr: GhPr =
            serde_json::from_value(value).unwrap_or_else(|error| panic!("parse PR: {error}"));

        let error = validate_get_pr_stack(&test_config(), &test_github(), 11, &[pr])
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("fork-backed PR should fail"));

        assert!(error.contains("fork-backed PR #11"), "{error}");
        assert!(error.contains("fork/repo"), "{error}");
    }

    #[test]
    fn sync_validation_rejects_unexpected_frozen_pr_retarget() {
        let bottom: GhPr = serde_json::from_str(&pr_json_with_details(
            11,
            "OPEN",
            "stack/bottom-title-bottomch",
        ...
        .unwrap_or_else(|error| panic!("parse bottom PR: {error}"));
        let top: GhPr = serde_json::from_str(&pr_json_with_details(
            12,
            "OPEN",
            "stack/top-title-topchang",
        ...
        .unwrap_or_else(|error| panic!("parse top PR: {error}"));
        let dependencies = vec![
            test_frozen_dependency(11, "1111111111111111111111111111111111111111", &["trunk"]),
            test_frozen_dependency(
                12,
                "2222222222222222222222222222222222222222",
                &["1111111111111111111111111111111111111111"],
            ),
        ...
        let error = validate_sync_frozen_pr_stack(
            &test_config(),
            &test_github(),
            &dependencies,
            &[bottom, top],
        ...
        .err()
        .map(|error| error.to_string())
        .unwrap_or_else(|| panic!("unexpected retarget should fail"));

        assert!(error.contains("unexpected retarget"), "{error}");
        assert!(error.contains("jj-stack/frozen/pr-12"), "{error}");
        assert!(error.contains("stack/other-base"), "{error}");
    }

    #[test]
    fn sync_validation_rejects_closed_unmerged_dependency() {
        let pr: GhPr = serde_json::from_str(&pr_json_with_details(
            11,
            "CLOSED",
            "stack/bottom-title-bottomch",
            DEFAULT_TRUNK,
        ...
        .unwrap_or_else(|error| panic!("parse PR: {error}"));
        let dependencies = vec![test_frozen_dependency(
            11,
            "1111111111111111111111111111111111111111",
            &["trunk"],
        )];
        ...
        let error =
            validate_sync_frozen_pr_stack(&test_config(), &test_github(), &dependencies, &[pr])
                .err()
                .map(|error| error.to_string())
                .unwrap_or_else(|| panic!("closed dependency should fail"));

        assert!(
            error.contains("closed-unmerged frozen dependency"),
            "{error}"
        );
        assert!(error.contains("jj-stack/frozen/pr-11"), "{error}");
    }

    #[test]
    fn get_rejects_divergent_frozen_bookmark_movement() {
        let pr: GhPr = serde_json::from_str(&pr_json_with_details(
            11,
            "OPEN",
            "stack/bottom-title-bottomch",
        ...
        .unwrap_or_else(|error| panic!("parse PR: {error}"));
        let runner = FakeRunner::new([
            fake_frozen_bookmarks(
                "jj-stack/frozen/pr-11\tok\t1111111111111111111111111111111111111111\n",
            ),
            fake_failure(
                "git",
                &[
        ...
        let error = update_get_frozen_bookmarks(&runner, &[pr], Diagnostics::default())
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("divergent frozen bookmark should fail"));
    ...
    #[test]
    fn stack_comment_pr_parser_preserves_order_and_deduplicates() {
        let body = concat!(
            "<!-- stack:v1 -->\n",
            "- [a #10](https://github.com/owner/repo/pull/10) _aaaaaaaa_\n",
            "- [b #11](https://github.com/owner/repo/pull/11) _bbbbbbbb_\n",
            "- [a again #10](https://github.com/owner/repo/pull/10) _aaaaaaaa_\n",
        );

        assert_eq!(parse_stack_pr_numbers(body), vec![10, 11]);
    }

    #[test]
    fn sync_defaults_to_no_submit_and_accepts_submit_option() -> Result<()> {
        let default = Cli::try_parse_from(["jj-stack", "sync"])?;
        let Commands::Sync(options) = default.command else {
            panic!("expected sync command");
        };
        assert!(!options.submit, "sync must not submit by default");

        let cli = Cli::try_parse_from(["jj-stack", "sync", "--submit"])?;
        let Commands::Sync(options) = cli.command else {
            panic!("expected sync command");
        };

        assert!(options.submit);
        assert_eq!(options.stack.revset, DEFAULT_STACK_REVSET);
        Ok(())
    }

    #[test]
    fn merge_accepts_target_option() -> Result<()> {
        let cli = Cli::try_parse_from(["jj-stack", "merge", "12"])?;
        let Commands::Merge(options) = cli.command else {
            panic!("expected merge command");
        };

        assert_eq!(options.target.as_deref(), Some("12"));
        assert_eq!(options.stack.revset, DEFAULT_STACK_REVSET);
        Ok(())
    }

    #[test]
    fn merge_target_pr_replaces_default_stack_head() -> Result<()> {
        ...
        let revset = effective_merge_revset(&runner, DEFAULT_STACK_REVSET, Some("12"))?;

        assert_eq!(
            revset,
            format!("trunk()..{head} & ~::(immutable_heads() | root()) & ~empty()")
        );
        Ok(())
    }

    #[test]
    fn merge_target_local_rev_falls_back_after_pr_lookup_miss() -> Result<()> {
        ...
        let revset = effective_merge_revset(&runner, DEFAULT_STACK_REVSET, Some("@-"))?;
    ...
    #[test]
    fn merge_target_intersects_custom_revset() {
        assert_eq!(
            merge_revset_for_target("trunk()..feature", "abc123"),
            "(trunk()..feature) & ::abc123"
        );
    }

    #[test]
    fn refresh_stack_above_merge_is_noop_when_nothing_remains() -> Result<()> {
        // When the targeted merge consumed the whole stack, the full revset
        // resolves to no changes; the refresh must not resolve context or submit.
        let runner = FakeRunner::new([fake_success(
            "jj",
            &[
                "log",
        ...
        assert_eq!(
            calls.len(),
            1,
            "only the stack lookup should run: {calls:?}"
        );
        assert!(
            !calls.iter().any(|call| call.starts_with("gh ")),
            "no GitHub submit work when nothing remains above the target: {calls:?}"
        );
        Ok(())
    }

    #[test]
    fn resolves_config_from_jj_before_git_before_defaults() -> Result<()> {
        let runner = FakeRunner::new([
            fake_success("jj", &["config", "get", "stack.remote"], "upstream"),
            fake_success("git", &["config", "--get", "stack.trunk"], "develop"),
            fake_success(
                "git",
                &["config", "--get", "stack.require-approval"],
                "false",
            ),
        ]);
    ...
    #[test]
    fn rejects_config_values_that_could_be_parsed_as_flags() {
        for bad in [
            "--insert-after",
            "main:evil",
            "feature branch",
            ...
            let result = validate_ref_component("trunk", bad.to_owned());
            assert!(result.is_err(), "expected `{bad}` to be rejected");
        }
        for good in ["main", "origin", "stack", "feature/foo-bar_1", "release.1"] {
            assert_eq!(
                validate_ref_component("trunk", good.to_owned()).unwrap(),
                good
            );
        }
    }
    ...
    #[test]
    fn resolve_rejects_injected_trunk_config() {
        let runner = FakeRunner::new([fake_success(
            "jj",
            &["config", "get", "stack.trunk"],
            "--insert-after",
        )]);

        let error = AppConfig::resolve(&runner).unwrap_err().to_string();
        assert!(
            error.contains("stack.trunk"),
            "expected trunk validation error, got `{error}`"
        );
    }

    #[test]
    fn startup_config_installs_default_immutable_aliases() -> Result<()> {
        let actions = plan_startup_config(None, JJ_DEFAULT_IMMUTABLE_ALIAS_VALUE, None)?;
    ...
    #[test]
    fn startup_config_is_idempotent() -> Result<()> {
        let actions = plan_startup_config(
            Some(JJ_FROZEN_ALIAS_VALUE),
            JJ_REQUIRED_IMMUTABLE_ALIAS_VALUE,
            None,
        )?;

        assert!(actions.is_empty(), "expected no config writes: {actions:?}");
        Ok(())
    }

    #[test]
    fn startup_config_wraps_custom_immutable_alias_once() -> Result<()> {
        let actions = plan_startup_config(Some(JJ_FROZEN_ALIAS_VALUE), "trunk() | tags()", None)?;
    ...
    #[test]
    fn startup_config_refuses_ambiguous_custom_immutable_alias() {
        let error = plan_startup_config(
            Some(JJ_FROZEN_ALIAS_VALUE),
            "trunk() | tags()",
            Some("old-custom()"),
        )
        .err()
        .map(|error| error.to_string())
        .unwrap_or_else(|| panic!("ambiguous custom config should fail"));

        assert!(error.contains("refusing to wrap ambiguous"), "{error}");
    }

    #[test]
    fn startup_config_unreadable_immutable_alias_fails_clearly() {
        let runner = FakeRunner::new([fake_failure(
            "jj",
            &["config", "get", JJ_CONFIG_IMMUTABLE_ALIAS_KEY],
            "config parse error",
        )]);
        let error = ensure_startup_config(
            &runner,
            Diagnostics {
                verbose: false,
                dry_run: false,
        ...
        .err()
        .map(|error| error.to_string())
        .unwrap_or_else(|| panic!("unreadable immutable config should fail"));

        assert!(error.contains("failed-command=`jj config get"), "{error}");
        assert!(error.contains("config parse error"), "{error}");
    }

    #[test]
    fn resolves_github_context_through_gh_commands() -> Result<()> {
        let runner = FakeRunner::new([
            fake_success(
                "gh",
                &[
    ...
    #[test]
    fn fake_runner_reports_unused_configured_commands() -> Result<()> {
        let used = fake_success("gh", &["api", "user", "--jq", ".login"], "octocat");
        let unused = fake_success("gh", &["api", "repos/owner/repo"], "{}");
        let runner = FakeRunner::new([used, unused]);

        run_required(&runner, "gh", &["api", "user", "--jq", ".login"])?;

        assert_eq!(
            runner.unused_configured_commands(),
            vec!["gh api repos/owner/repo".to_owned()]
        );
        Ok(())
    }

    #[test]
    fn resolves_stack_with_machine_readable_template_in_bottom_to_top_order() -> Result<()> {
        let bottom_commit = "1111111111111111111111111111111111111111";
        let top_commit = "2222222222222222222222222222222222222222";
        let runner = FakeRunner::new([
            fake_success(
                "jj",
                &[
            ...
            fake_success(
                "git",
                &["show", "-s", "--format=%T", bottom_commit],
                "bottomtree",
            ),
            fake_success("git", &["show", "-s", "--format=%T", top_commit], "toptree"),
        ]);
        ...
        assert_eq!(
            ...
            vec![
                ...
                display_command("git", &["show", "-s", "--format=%T", bottom_commit]),
                display_command("git", &["show", "-s", "--format=%T", top_commit]),
            ]
        );
        assert_eq!(
            stack,
            vec![
                ResolvedChange {
                    change_id: "bottomchange".to_owned(),
                    commit_id: bottom_commit.to_owned(),
                    parent_ids: vec!["0000000000000000000000000000000000000000".to_owned()],
                    title: "bottom title".to_owned(),
                    body: "bottom body".to_owned(),
                    tree_id: "bottomtree".to_owned(),
                    empty: false,
                    conflict: false,
                },
                ResolvedChange {
                    change_id: "topchange".to_owned(),
                    commit_id: top_commit.to_owned(),
                    parent_ids: vec![bottom_commit.to_owned()],
                    title: "top title".to_owned(),
                    body: "line one\nline two".to_owned(),
                    tree_id: "toptree".to_owned(),
                    empty: false,
                    conflict: true,
                },
            ]
    ...
    #[test]
    fn resolves_trunk_revset_to_exactly_one_commit() -> Result<()> {
        ...
        assert_eq!(resolve_single_rev(&runner, "trunk()")?, commit);
        Ok(())
    }

    #[test]
    fn rejects_missing_or_ambiguous_trunk_revset() {
        let missing = FakeRunner::new([fake_success(
            "jj",
            &[
                "log",
                "--no-graph",
        ...
        let error = resolve_single_rev(&missing, "trunk()")
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("missing trunk should fail"));
        assert!(error.contains("resolved to no commits"), "{error}");

        let ambiguous = FakeRunner::new([fake_success(
            "jj",
            &[
                "log",
                "--no-graph",
        ...
        let error = resolve_single_rev(&ambiguous, "trunk()")
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("ambiguous trunk should fail"));
        assert!(error.contains("resolved to 2 commits"), "{error}");
    }

    #[test]
    fn detects_frozen_pr_bookmarks_only() -> Result<()> {
        let runner = FakeRunner::new([fake_frozen_bookmarks(
            "jj-stack/frozen/pr-12\tok\t1111111111111111111111111111111111111111\n\
             other/frozen/pr-99\tok\t9999999999999999999999999999999999999999\n\
             jj-stack/frozen/pr-7\tok\t7777777777777777777777777777777777777777\n",
        )]);

        let bookmarks = frozen_bookmarks(&runner)?;

        assert_eq!(
            bookmarks,
            vec![
                FrozenBookmark {
                    name: "jj-stack/frozen/pr-7".to_owned(),
                    pr_number: 7,
                    commit_id: "7777777777777777777777777777777777777777".to_owned(),
                },
                FrozenBookmark {
                    name: "jj-stack/frozen/pr-12".to_owned(),
                    pr_number: 12,
    ...
    #[test]
    fn rejects_malformed_or_conflicted_frozen_bookmarks() {
        let malformed = FakeRunner::new([fake_frozen_bookmarks(
            "jj-stack/frozen/not-pr\tok\t1111111111111111111111111111111111111111\n\
             jj-stack/frozen/pr-nope\tok\t2222222222222222222222222222222222222222\n",
        )]);
        let error = frozen_bookmarks(&malformed)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("malformed frozen bookmark should fail"));
        assert!(error.contains("parse frozen bookmark"), "{error}");

        let conflicted = FakeRunner::new([fake_frozen_bookmarks(
            "jj-stack/frozen/pr-12\tconflicted\t\n",
        )]);
        let error = frozen_bookmarks(&conflicted)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("conflicted frozen bookmark should fail"));
        assert!(error.contains("is conflicted"), "{error}");
    }

    #[test]
    fn finds_nearest_frozen_boundary_below_owned_stack() -> Result<()> {
        let frozen_commit = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let owned = vec![test_change(
            "owned",
            "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
            &[frozen_commit],
        )];
        let dependencies = frozen_dependencies_below_owned(
            &owned,
            vec![test_frozen_dependency(11, frozen_commit, &["trunk"])],
        )?;

        assert_eq!(dependency_pr_numbers(&dependencies), vec![11]);
        Ok(())
    }

    #[test]
    fn finds_full_ordered_frozen_dependency_list_below_owned_stack() -> Result<()> {
        let bottom_frozen = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let top_frozen = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let unrelated_frozen = "cccccccccccccccccccccccccccccccccccccccc";
        let owned = vec![test_change(
            "owned",
        ...
        let dependencies = frozen_dependencies_below_owned(
            &owned,
            vec![
                test_frozen_dependency(12, top_frozen, &[bottom_frozen]),
                test_frozen_dependency(10, unrelated_frozen, &["trunk"]),
                test_frozen_dependency(11, bottom_frozen, &["trunk"]),
            ],
        )?;

        assert_eq!(dependency_pr_numbers(&dependencies), vec![11, 12]);
        Ok(())
    }

    #[test]
    fn rejects_multiple_nearest_frozen_boundaries() {
        let left_frozen = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let right_frozen = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let owned = vec![test_change(
            "owned",
        ...
        let error = frozen_dependencies_below_owned(
            &owned,
            vec![
                test_frozen_dependency(11, left_frozen, &["trunk"]),
                test_frozen_dependency(12, right_frozen, &["trunk"]),
            ],
        )
        .err()
        .map(|error| error.to_string())
        .unwrap_or_else(|| panic!("multiple frozen boundaries should fail"));

        assert!(error.contains("multiple frozen boundaries"), "{error}");
    ...
    #[test]
    fn generic_immutable_history_is_not_a_frozen_dependency() -> Result<()> {
        let owned = vec![test_change(
            "owned",
            "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
            &["aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"],
    ...
    #[test]
    fn accepts_linear_stack_shape() -> Result<()> {
        validate_stack_shape(&linear_stack(), DEFAULT_STACK_REVSET)
    }

    #[test]
    fn rejects_multiple_roots() {
        assert_rejects_shape(
            &[
                test_change(
                    "rootone",
                    "1111111111111111111111111111111111111111",
        ...
        let error = validate_stack_shape(
            &[
                test_change(
                    "rootone",
                    "1111111111111111111111111111111111111111",
        ...
        .err()
        .map(|error| error.to_string())
        .unwrap_or_else(|| panic!("multiple roots should fail"));
        assert!(error.contains("rootone"), "{error}");
        assert!(error.contains("roottwo"), "{error}");
    }

    #[test]
    fn rejects_siblings() {
        let root_commit = "1111111111111111111111111111111111111111";

        assert_rejects_shape(
            &[
                test_change("root", root_commit, &["trunk"]),
                test_change(
                    "left",
                    "2222222222222222222222222222222222222222",
                    &[root_commit],
        ...
        .err()
        .map(|error| error.to_string())
        .unwrap_or_else(|| panic!("siblings should fail"));
    ...
    #[test]
    fn rejects_conflicted_changes() {
        let mut stack = linear_stack();
        stack[1].conflict = true;

        assert_rejects_shape(&stack, "conflicted change");
    }
    ...
    fn rejects_empty_changes() {
        ...
        assert_rejects_shape(&stack, "empty change");
    }

    #[test]
    fn rejected_stack_does_not_touch_github() {
        let commit = "1111111111111111111111111111111111111111";
        let runner = FakeRunner::new([
            fake_success(
                "jj",
        ...
        let cli =
            Cli::try_parse_from(["jj-stack", "submit"]).unwrap_or_else(|error| panic!("{error}"));

        let result = run(cli, &runner);

        assert!(result.is_err(), "conflicted stack should be rejected");
        let message = result
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| "missing error".to_owned());
        assert!(
            message.contains("conflicted change"),
            "error should name conflicted change shape: {message}"
        );
        assert!(
            runner.calls().iter().all(|call| !call.starts_with("gh ")),
            "rejected stack should not call gh: {:?}",
            runner.calls()
        );
    }

    #[test]
    fn conflicted_frozen_bookmark_blocks_stack_operations_before_github() {
        let runner = FakeRunner::new([fake_frozen_bookmarks(
            "jj-stack/frozen/pr-12\tconflicted\t\n",
        )]);
        let cli =
        ...
        let message = result
            ...
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("conflicted frozen bookmark should fail"));
        assert!(message.contains("jj-stack/frozen/pr-12"), "{message}");
        assert!(
            runner.calls().iter().all(|call| !call.starts_with("gh ")),
            "frozen bookmark failure should stop before GitHub: {:?}",
            runner.calls()
        );
    }

    #[test]
    fn resolves_cache_path_under_default_jj_repo_dir() -> Result<()> {
        let workspace = unique_test_dir("default-jj-repo");
    ...
    #[test]
    fn resolves_cache_path_through_non_default_workspace_pointer() -> Result<()> {
        let root = unique_test_dir("pointer-jj-repo");
        let primary_repo = root.join("primary").join(".jj").join("repo");
        let workspace_jj = root.join("workspace").join(".jj");
        fs::create_dir_all(&primary_repo)?;
        fs::create_dir_all(&workspace_jj)?;
        fs::write(workspace_jj.join("repo"), "../../primary/.jj/repo")?;
    ...
    #[test]
    fn saves_and_loads_repo_private_sqlite_cache() -> Result<()> {
        let root = unique_test_dir("cache-sqlite");
        let cache_path = root.join("stack").join("cache.sqlite");
        let mut store = CacheStore::load(cache_path.clone())?;
        let entry = test_pr_entry(12, "stack/change-title-changeon");

        store.upsert_pr("owner/repo", "changeone", entry.clone());
        store.save()?;

        assert!(cache_path.exists(), "cache.sqlite should be created");
        let loaded = CacheStore::load(cache_path)?;
        assert_eq!(loaded.get_pr("owner/repo", "changeone"), Some(&entry));
        fs::remove_dir_all(root)?;
        Ok(())
    }

    #[test]
    fn sqlite_cache_save_rolls_back_on_insert_failure() -> Result<()> {
        let root = unique_test_dir("cache-transaction");
        let cache_path = root.join("stack").join("cache.sqlite");
        let original = test_pr_entry(12, "stack/original");
        let mut store = CacheStore::load(cache_path.clone())?;
        store.upsert_pr("owner/repo", "changeone", original.clone());
        store.save()?;
        let conn = Connection::open(&cache_path)?;
        conn.execute_batch(
            "CREATE TRIGGER pr_cache_insert_abort
             BEFORE INSERT ON pr_cache
             BEGIN
        ...
        store.upsert_pr(
            "owner/repo",
            "changeone",
            test_pr_entry(12, "stack/updated"),
        );
        let error = store
            .save()
            .err()
            .map(|error| format!("{error:#}"))
            .unwrap_or_else(|| panic!("trigger should fail cache save"));

        assert!(error.contains("forced insert failure"), "{error}");
        let loaded = CacheStore::load(cache_path)?;
        assert_eq!(loaded.get_pr("owner/repo", "changeone"), Some(&original));
        fs::remove_dir_all(root)?;
        Ok(())
    }

    #[test]
    fn old_state_json_is_ignored() -> Result<()> {
        let root = unique_test_dir("ignore-state-json");
        let stack_dir = root.join("stack");
        ...
        fs::write(
            ...
            serde_json::to_string(&serde_json::json!({
                "repos": {
                    "owner/repo": {
                        "changes": {
                            "changeone": test_pr_entry(12, "stack/old-json")
                        }
                    }
                }
            }))?,
        ...
        let loaded = CacheStore::load(stack_dir.join("cache.sqlite"))?;

        assert!(loaded.get_pr("owner/repo", "changeone").is_none());
        assert!(
            !stack_dir.join("cache.sqlite").exists(),
            "read-only load of missing SQLite cache should not create a file"
        );
        fs::remove_dir_all(root)?;
        Ok(())
    }
    ...
    #[test]
    fn malformed_sqlite_cache_loads_as_empty_in_best_effort_mode() -> Result<()> {
        let workspace = test_workspace("malformed-cache-load")?;
        ...
        fs::write(&cache_path, "not a sqlite database")?;
        let runner = FakeRunner::new([fake_success(
            "jj",
            &["root"],
            workspace
        ...
        let store = CacheStore::load_current_best_effort(
            &runner,
            Diagnostics {
                verbose: true,
                dry_run: false,
        ...
        assert!(store.get_pr("owner/repo", "changeone").is_none());
        fs::remove_dir_all(workspace)?;
        Ok(())
    }

    #[test]
    fn cache_write_failure_is_best_effort() -> Result<()> {
        let root = unique_test_dir("cache-write-failure");
        let cache_path = root.join("stack").join("cache.sqlite");
        fs::write(root.join("stack"), "not a directory")?;
        let mut store = CacheStore::load(cache_path)?;
        store.upsert_pr("owner/repo", "changeone", test_pr_entry(9, "stack/change"));

        let saved = store.save_best_effort(
            Diagnostics {
                verbose: true,
                dry_run: false,
            },
        ...
        assert!(!saved, "invalid cache path should warn and continue");
        fs::remove_dir_all(root)?;
        Ok(())
    }

    #[test]
    fn lookup_uses_local_cache_before_branch_search() -> Result<()> {
        let github = test_github();
        let mut store = test_store();
        store.upsert_pr(
            "owner/repo",
            "changeone",
            test_pr_entry(7, "stack/change-title-changeon"),
        );
        let runner = FakeRunner::new([fake_pr_fetch(
            7,
            &pr_json(
        ...
        .unwrap_or_else(|| panic!("cache-backed PR should resolve"));

        assert_eq!(entry.pr_number, 7);
        assert_eq!(entry.head_sha, "headsha");
        assert_eq!(entry.stack_comment_id, Some("comment-123".to_owned()));
        assert_eq!(
            runner.calls(),
            vec![display_command(
                "gh",
                &["api", "repos/owner/repo/pulls/7", "--jq", PR_API_JQ],
            )]
        );
        Ok(())
    }

    fn fake_branch_pr_list(stdout: &str) -> (String, CommandOutput) {
        fake_success(
            "gh",
            &[
                "pr",
    ...
    #[test]
    fn lookup_falls_back_to_open_pr_with_deterministic_head_branch() -> Result<()> {
        let github = test_github();
        let store = test_store();
        let runner = FakeRunner::new([
            fake_branch_pr_list(&format!(
                "[{}]",
                pr_json(
                    9,
                    "OPEN",
            ...
            // No matching stack comment exists yet.
            fake_comment_list(9, ""),
        ]);
        ...
        .unwrap_or_else(|| panic!("branch-backed PR should resolve"));

        assert_eq!(entry.pr_number, 9);
        assert_eq!(entry.head_branch, "stack/change-title-changeon");
        assert_eq!(entry.stack_comment_id, None);
        Ok(())
    }

    #[test]
    fn branch_recovery_backfills_existing_stack_comment_id() -> Result<()> {
        let github = test_github();
        let store = test_store();
        let body = format!("{STACK_COMMENT_MARKER}\nStack for owner/repo");
        let runner = FakeRunner::new([
            fake_branch_pr_list(&format!(
                "[{}]",
                pr_json(
            ...
            fake_comment_list(
                9,
                &comment_line(555, &body, "octocat", "2026-06-03T00:00:00Z"),
            ),
        ]);
        ...
        assert_eq!(entry.stack_comment_id.as_deref(), Some("555"));
        Ok(())
    }

    #[test]
    fn closed_cache_falls_back_to_branch_lookup() -> Result<()> {
        let github = test_github();
        let mut store = test_store();
        store.upsert_pr(
            "owner/repo",
        ...
        .unwrap_or_else(|| panic!("branch lookup should recover closed cache"));
    ...
    #[test]
    fn missing_cache_falls_back_to_branch_lookup() -> Result<()> {
        let github = test_github();
        let mut store = test_store();
        store.upsert_pr(
            "owner/repo",
        ...
        .unwrap_or_else(|| panic!("branch lookup should recover missing cache"));
    ...
    #[test]
    fn missing_github_metadata_falls_back_to_branch_lookup() -> Result<()> {
        let github = test_github();
        let mut store = test_store();
        let mut entry = test_pr_entry(7, "stack/change-title-changeon");
        entry.pr_node_id.clear();
        store.upsert_pr("owner/repo", "changeone", entry);
        let runner = FakeRunner::new([
            fake_pr_fetch(
                7,
                &pr_json(
        ...
        .unwrap_or_else(|| panic!("branch lookup should recover incomplete cache metadata"));
    ...
    #[test]
    fn github_metadata_mismatch_falls_back_to_branch_lookup() -> Result<()> {
        let github = test_github();
        let mut store = test_store();
        let mut entry = test_pr_entry(7, "stack/change-title-changeon");
        entry.head_repo_node_id = "old-repo-node".to_owned();
        store.upsert_pr("owner/repo", "changeone", entry);
        let runner = FakeRunner::new([
            fake_pr_fetch(
                7,
        ...
        .unwrap_or_else(|| panic!("branch lookup should recover mismatched cache metadata"));
    ...
    #[test]
    fn github_pr_head_mismatch_falls_back_to_branch_lookup() -> Result<()> {
        let github = test_github();
        let mut store = test_store();
        store.upsert_pr(
            "owner/repo",
        ...
        .unwrap_or_else(|| panic!("branch lookup should recover head mismatch"));

        assert_eq!(entry.pr_number, 9);
        assert!(runner.calls().iter().all(|call| !call.contains("push")));
        Ok(())
    }

    #[test]
    fn conflicting_cache_falls_back_to_branch_lookup() -> Result<()> {
        let github = test_github();
        let mut store = test_store();
        store.upsert_pr("owner/repo", "changeone", test_pr_entry(7, "stack/other"));
        let runner = FakeRunner::new([
            fake_branch_pr_list(&format!(
                "[{}]",
                pr_json(
        ...
        .unwrap_or_else(|| panic!("branch lookup should recover conflicting cache"));
    ...
    #[test]
    fn conflicting_branch_lookup_errors_instead_of_choosing_pr() {
        let github = test_github();
        let store = test_store();
        let runner = FakeRunner::new([fake_success(
            "gh",
        ...
        .err()
        .map(|error| error.to_string())
        .unwrap_or_else(|| panic!("conflicting branch lookup should fail"));

        assert!(message.contains("conflicting PR lookup"), "{message}");
        assert!(message.contains("2 open PRs"), "{message}");
    }

    #[test]
    fn deterministic_head_branch_uses_configured_prefix_and_title_slug() {
        let config = AppConfig {
            remote: DEFAULT_REMOTE.to_owned(),
            trunk: DEFAULT_TRUNK.to_owned(),
            require_approval: DEFAULT_REQUIRE_APPROVAL,
            branch_prefix: "stack".to_owned(),
        };
        let mut change = test_change("changeone", "headsha", &["basesha"]);
        change.title = " Hello  World! ".to_owned();

        assert_eq!(
            deterministic_head_branch(&config, &change, &HashSet::new()),
            "stack/hello-world-changeon"
        );
    }

    #[test]
    fn change_id_branch_prefix_uses_first_eight_chars() {
        assert_eq!(
            change_id_branch_prefix("pmtxtmluxqzxvksysypyzxqumkuvvlyo"),
            "pmtxtmlu"
        );
        assert_eq!(change_id_branch_prefix("short"), "short");
    }

    #[test]
    fn deterministic_head_branch_turns_punctuation_into_separators() {
        let config = AppConfig {
            remote: DEFAULT_REMOTE.to_owned(),
            trunk: DEFAULT_TRUNK.to_owned(),
            require_approval: DEFAULT_REQUIRE_APPROVAL,
        ...
        let mut change = test_change("changeone", "headsha", &["basesha"]);
        change.title = "fix(types): restore repo check-types".to_owned();

        assert_eq!(
            deterministic_head_branch(&config, &change, &HashSet::new()),
            "stack/fix-types-restore-repo-check-types-changeon"
        );
    }

    #[test]
    fn deterministic_head_branch_suffixes_duplicate_title_slugs() {
        let config = AppConfig {
            remote: DEFAULT_REMOTE.to_owned(),
            trunk: DEFAULT_TRUNK.to_owned(),
            require_approval: DEFAULT_REQUIRE_APPROVAL,
        ...
        let mut used = HashSet::from(["stack/change-title-changeon".to_owned()]);
        let mut change = test_change("changeone", "headsha", &["basesha"]);
        change.title = "change title".to_owned();

        assert_eq!(
            deterministic_head_branch(&config, &change, &used),
            "stack/change-title-changeon-1"
        );

        used.insert("stack/change-title-changeon-1".to_owned());
        assert_eq!(
            deterministic_head_branch(&config, &change, &used),
            "stack/change-title-changeon-2"
        );
    }

    #[test]
    fn submit_head_branch_preserves_existing_cache_branch() -> Result<()> {
        let config = test_config();
        let github = test_github();
        let mut store = test_store();
        let old_branch = "stack/old-readable-branch";
        let commit = "1111111111111111111111111111111111111111";
        store.upsert_pr(
            "owner/repo",
            "changeone",
            PrCacheEntry {
                pr_number: 9,
                head_branch: old_branch.to_owned(),
                base_branch: DEFAULT_TRUNK.to_owned(),
                base_ref: DEFAULT_TRUNK.to_owned(),
                head_sha: commit.to_owned(),
                base_sha: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa".to_owned(),
                title: "old title".to_owned(),
                body: String::new(),
                created_at: "2026-06-03T12:34:56Z".to_owned(),
                stack_comment_id: None,
                ..test_pr_entry(9, old_branch)
            },
        );
        ...
        let context = AppContext::from_owned(github.clone(), vec![change.clone()]);
        let mut used = HashSet::new();
        let runner = FakeRunner::new([
            fake_local_bookmark(old_branch, commit),
            fake_tracked_remote_bookmark(old_branch),
            fake_pr_fetch(
                9,
                &pr_json_with_details(
                    9,
            ...
            fake_success(
                "git",
                &["ls-remote", "--heads", DEFAULT_REMOTE, old_branch],
                &format!("{commit}\trefs/heads/{old_branch}"),
            ),
        ]);

        let (head_branch, existing_pr, remote_head) = resolve_submit_head_branch(
            &runner,
            &config,
            &mut used,
            &store,
        ...
        assert_eq!(head_branch, old_branch);
        assert_eq!(existing_pr.map(|entry| entry.pr_number), Some(9));
        assert_eq!(remote_head.as_deref(), Some(commit));
        assert!(used.contains(old_branch));
        assert!(
            runner
                .calls()
                .iter()
                .all(|call| !call.contains("change-title-changeon")),
            "cache-backed lookup should not use the new branch formula: {:?}",
            runner.calls()
        );
        Ok(())
    }
    ...
    #[test]
    fn submit_head_branch_refuses_remote_branch_without_cache() -> Result<()> {
        let config = test_config();
        let github = test_github();
        let store = test_store();
        let commit = "1111111111111111111111111111111111111111";
        ...
        let branch = "stack/changeone-title-changeon";
        let context = AppContext::from_owned(github, vec![change.clone()]);
        let mut used = HashSet::new();
        let runner = FakeRunner::new([
            fake_local_stack_bookmarks(&change, []),
            fake_success(
                "gh",
                &[
                    "pr",
            ...
            fake_success(
                "git",
                &["ls-remote", "--heads", DEFAULT_REMOTE, branch],
                &format!("{commit}\trefs/heads/{branch}"),
            ),
        ]);

        let error = resolve_submit_head_branch(
            &runner,
            &config,
            &mut used,
            &store,
        ...
        .err()
        .map(|error| error.to_string())
        .unwrap_or_else(|| panic!("remote branch without cache should fail"));

        assert!(
            error.contains("does not identify a safe matching PR"),
            "{error}"
        );
    ...
    #[test]
    fn submit_head_branch_discovers_existing_pr_from_tracked_local_bookmark() -> Result<()> {
        let config = test_config();
        let github = test_github();
        let store = test_store();
        let old_commit = "9999999999999999999999999999999999999999";
        ...
        change.title = "retitled locally".to_owned();
        let context = AppContext::from_owned(github.clone(), vec![change.clone()]);
        let mut used = HashSet::new();
        let runner = FakeRunner::new([
            fake_local_stack_bookmarks(&change, [branch]),
            fake_success(
                "gh",
                &[
                    "pr",
            ...
            fake_comment_list(9, ""),
            fake_local_bookmark(branch, new_commit),
            fake_tracked_remote_bookmark(branch),
            fake_success(
                "git",
                &["ls-remote", "--heads", DEFAULT_REMOTE, branch],
                &format!("{old_commit}\trefs/heads/{branch}"),
            ),
        ]);
        ...
        assert_eq!(head_branch, branch);
        assert_eq!(existing_pr.map(|entry| entry.pr_number), Some(9));
        assert_eq!(remote_head.as_deref(), Some(old_commit));
        assert!(used.contains(branch));
        assert!(
            runner
                .calls()
                .iter()
                .all(|call| !call.contains("retitled-locally")),
            "tracked bookmark discovery should not depend on current title slug: {:?}",
            runner.calls()
        );
        Ok(())
    }
    ...
    #[test]
    fn submit_head_branch_ignores_stale_cache_when_tracked_bookmark_has_live_pr() -> Result<()> {
        let config = test_config();
        let github = test_github();
        let mut store = test_store();
        ...
        store.upsert_pr(
            ...
            PrCacheEntry {
                pr_number: 8,
                head_branch: stale_branch.to_owned(),
                base_branch: DEFAULT_TRUNK.to_owned(),
                base_ref: DEFAULT_TRUNK.to_owned(),
                head_sha: old_commit.to_owned(),
                base_sha: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa".to_owned(),
                title: "stale title".to_owned(),
                body: String::new(),
                created_at: "2026-06-03T12:34:56Z".to_owned(),
                stack_comment_id: Some("stale-comment".to_owned()),
                ..test_pr_entry(8, stale_branch)
            },
        );
        let change = test_change(
            "changeone",
        ...
        let mut used = HashSet::new();
        let runner = FakeRunner::new([
            fake_local_stack_bookmarks(&change, [live_branch]),
            fake_success(
                "gh",
                &[
                    "pr",
            ...
            fake_comment_list(9, ""),
            fake_local_bookmark(live_branch, new_commit),
            fake_tracked_remote_bookmark(live_branch),
            fake_success(
                "git",
                &["ls-remote", "--heads", DEFAULT_REMOTE, live_branch],
                &format!("{old_commit}\trefs/heads/{live_branch}"),
            ),
        ]);
        ...
        assert_eq!(head_branch, live_branch);
        assert_eq!(existing_pr.map(|entry| entry.pr_number), Some(9));
        assert_eq!(remote_head.as_deref(), Some(old_commit));
        assert!(
            runner
                .calls()
                .iter()
                .all(|call| !call.contains(stale_branch)),
            "live bookmark discovery should not query stale cache branch: {:?}",
            runner.calls()
        );
        Ok(())
    }
    ...
    #[test]
    fn submit_validation_uses_tracked_bookmarks_without_cache_bookmark_metadata() {
        let entry = test_pr_entry(9, "stack/change-title-changeon");
        let change = test_change("changeone", "headsha", &["basesha"]);
        let runner = FakeRunner::new([
            fake_local_bookmark("stack/change-title-changeon", "headsha"),
            fake_tracked_remote_bookmark("stack/change-title-changeon"),
        ]);

        validate_submit_bookmark_state(&runner, &test_config(), &change, &entry)
            .unwrap_or_else(|error| panic!("tracked bookmarks should be enough: {error}"));
    }

    #[test]
    fn submit_bookmark_validation_requires_local_bookmark_at_selected_change() {
        let entry = test_pr_entry(9, "stack/change-title-changeon");
        let change = test_change("changeone", "headsha", &["basesha"]);
        let runner = FakeRunner::new([fake_local_bookmark(
            "stack/change-title-changeon",
            "othersha",
        )]);

        let error = validate_submit_bookmark_state(&runner, &test_config(), &change, &entry)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("wrong local bookmark target should fail"));

        assert!(error.contains("points at othersha"), "{error}");
        assert!(
            error.contains("selected change changeone is headsha"),
            "{error}"
        );
    }

    #[test]
    fn submit_bookmark_validation_requires_tracked_unconflicted_remote_bookmark() {
        let entry = test_pr_entry(9, "stack/change-title-changeon");
        let change = test_change("changeone", "headsha", &["basesha"]);
        let untracked = FakeRunner::new([
            fake_local_bookmark("stack/change-title-changeon", "headsha"),
            fake_remote_bookmark_status("stack/change-title-changeon", false, false),
        ]);

        let error = validate_submit_bookmark_state(&untracked, &test_config(), &change, &entry)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("untracked remote bookmark should fail"));
        assert!(error.contains("is untracked"), "{error}");

        let conflicted = FakeRunner::new([
            fake_local_bookmark("stack/change-title-changeon", "headsha"),
            fake_remote_bookmark_status("stack/change-title-changeon", true, true),
        ]);
        let error = validate_submit_bookmark_state(&conflicted, &test_config(), &change, &entry)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("conflicted bookmark should fail"));
        assert!(error.contains("is conflicted"), "{error}");
    }

    #[test]
    fn github_pr_parsing_treats_null_body_as_empty() -> Result<()> {
        let pr: GhPr = serde_json::from_value(serde_json::json!({
            "number": 9,
            "state": "OPEN",
            "headRefName": "stack/change-title-changeon",
            "baseRefName": "main",
        ...
        let merge_pr: GhMergePr = serde_json::from_value(serde_json::json!({
            "number": 9,
            "state": "OPEN",
            "headRefName": "stack/change-title-changeon",
            "baseRefName": "main",
        ...
        assert_eq!(pr.body, "");
        assert_eq!(merge_pr.body, "");
        Ok(())
    }

    #[test]
    fn submit_creates_prs_with_parent_head_branch_bases_after_jj_bookmark_push() -> Result<()> {
        let workspace = test_workspace("submit-create")?;
        let root_parent = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let bottom_commit = "1111111111111111111111111111111111111111";
        let top_commit = "2222222222222222222222222222222222222222";
        ...
        let runner = FakeRunner::new([
            ...
            fake_success(
                "git",
                &["merge-base", bottom_commit, DEFAULT_TRUNK],
                root_parent,
            ),
            fake_success(
                "git",
                &["merge-base", top_commit, bottom_commit],
                bottom_commit,
            ),
            fake_success(
                "gh",
            ...
            fake_success(
                "git",
                &["ls-remote", "--heads", DEFAULT_REMOTE, bottom_branch],
                "",
            ),
            fake_success(
                "gh",
            ...
            fake_success(
                "git",
                &["ls-remote", "--heads", DEFAULT_REMOTE, top_branch],
                "",
            ),
            fake_success(
                "jj",
            ...
            fake_success(
                ...
                &[
                    ...
                    "title=bottom title",
                    "-f",
                    "head=stack/bottom-title-bottomch",
                    "-f",
                    "base=main",
                    "-f",
                    "body=bottom body",
            ...
            fake_success(
                ...
                &[
                    ...
                    "title=top title",
                    "-f",
                    "head=stack/top-title-topchang",
                    "-f",
                    "base=stack/bottom-title-bottomch",
                    "-f",
                    "body=top body",
                    "--jq",
                    PR_API_JQ,
            ...
            fake_comment_list(11, ""),
            fake_comment_create(
                11,
                &expected_stack_comment_body(
                    &[
                        (
                            "bottomchange",
                            11,
        ...
        let cli = Cli::try_parse_from(["jj-stack", "submit"])?;

        run(cli, &runner)?;

        let calls = runner.calls();
        let push_index = command_index(
            &calls,
            "jj git push --remote origin --bookmark stack/bottom-title-bottomch",
        );
        let first_post_index = calls
            .iter()
            .position(|call| call.starts_with("gh api -X POST repos/owner/repo/pulls"))
            .unwrap_or_else(|| panic!("submit should create PRs through gh api POST: {calls:?}"));
        assert!(
            push_index < first_post_index,
            "submit should push jj bookmarks before creating PRs: {calls:?}"
        );
        assert!(
            calls.iter().all(|call| !call.starts_with("git push ")),
            "submit should not raw-push refs: {calls:?}"
        );
        ...
        assert_eq!(
            store
                .get_pr("owner/repo", "topchange")
                .map(|entry| entry.base_branch.as_str()),
            Some(bottom_branch)
        );
    ...
    #[test]
    fn submit_uses_frozen_dependency_branch_as_first_owned_base() -> Result<()> {
        let workspace = test_workspace("submit-frozen-base")?;
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let frozen_bottom_commit = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let frozen_top_commit = "cccccccccccccccccccccccccccccccccccccccc";
        let owned_commit = "1111111111111111111111111111111111111111";
        let frozen_bottom_branch = "stack/dependency-bottom-frozen20";
        let frozen_top_branch = "stack/dependency-top-frozen21";
        let owned_branch = "stack/owned-title-ownedcha";
        let mut owned = test_change("ownedchange", owned_commit, &[frozen_top_commit]);
        owned.title = "owned title".to_owned();
        owned.body = "owned body".to_owned();
        let frozen_bottom = test_frozen_dependency(20, frozen_bottom_commit, &[trunk]);
        let frozen_top = test_frozen_dependency(21, frozen_top_commit, &[frozen_bottom_commit]);
        let context = AppContext::new(
            test_github(),
            StackResolution {
                owned: vec![owned],
                frozen_dependencies: vec![frozen_bottom, frozen_top],
            },
        );
        let expected_body = concat!(
            ...
            "Stack for owner/repo\n\n",
            "Current stack:\n",
            "- **[owned title #22](https://github.com/owner/repo/pull/22)** _ownedcha_ · 2026-06-03 12:34:56 👈\n",
            "\n",
            "Dependencies:\n",
            "- [frozen21 title #21](https://github.com/owner/repo/pull/21) _frozen21_ · 2026-06-03 12:34:56\n",
            "- [frozen20 title #20](https://github.com/owner/repo/pull/20) _frozen20_ · 2026-06-03 12:34:56\n",
            "- main\n",
            "\n",
            "Check out this stack: `jj-stack get https://github.com/owner/repo/pull/22`\n",
        ...
        let runner = FakeRunner::new([
            fake_success(
                "git",
                &["merge-base", owned_commit, frozen_top_commit],
                frozen_top_commit,
            ),
            fake_success(
                "jj",
            ...
            fake_pr_fetch(
                ...
                &pr_json_with_details(
                    20,
                    "OPEN",
                    frozen_bottom_branch,
                    DEFAULT_TRUNK,
                    frozen_bottom_commit,
                    trunk,
                    "frozen20 title",
                    "",
                ),
            ...
            fake_success(
                "git",
                &["ls-remote", "--heads", DEFAULT_REMOTE, owned_branch],
                "",
            ),
            fake_success(
                "jj",
            ...
            fake_success(
                ...
                &[
                    ...
                    "title=owned title",
                    "-f",
                    "head=stack/owned-title-ownedcha",
                    "-f",
                    "base=stack/dependency-top-frozen21",
                    "-f",
                    "body=owned body",
                    "--jq",
                    PR_API_JQ,
            ...
            fake_comment_list(22, ""),
            fake_comment_create(22, expected_body, 222),
        ]);

        submit_stack(&runner, &test_config(), &context, Diagnostics::default())?;

        let calls = runner.calls();
        assert!(
            calls
                .iter()
                .any(|call| { call.contains("-f base=stack/dependency-top-frozen21") }),
            "owned PR should target the nearest frozen dependency branch: {calls:?}"
        );
        assert!(
            calls.iter().all(|call| !call
                .starts_with("gh api --paginate repos/owner/repo/issues/20/comments")
                && !call.starts_with("gh api --paginate repos/owner/repo/issues/21/comments")),
            "submit should not touch frozen dependency comments: {calls:?}"
        );
        assert!(
            calls
                .iter()
                .all(|call| !call.contains("bookmark stack/dependency")),
            "submit should not move frozen dependency branches: {calls:?}"
        );
    ...
    #[test]
    fn submit_uses_single_frozen_dependency_branch_as_first_owned_base() -> Result<()> {
        let workspace = test_workspace("submit-one-frozen-base")?;
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let frozen_commit = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let owned_commit = "1111111111111111111111111111111111111111";
        let frozen_branch = "stack/dependency-frozen20";
        let owned_branch = "stack/owned-title-ownedcha";
        let mut owned = test_change("ownedchange", owned_commit, &[frozen_commit]);
        owned.title = "owned title".to_owned();
        owned.body = "owned body".to_owned();
        let frozen = test_frozen_dependency(20, frozen_commit, &[trunk]);
        let context = AppContext::new(
            test_github(),
            StackResolution {
                owned: vec![owned],
                frozen_dependencies: vec![frozen],
            },
        );
        let expected_body = concat!(
            "<!-- stack:v1 -->\n",
        ...
        let runner = FakeRunner::new([
            fake_success(
                "git",
                &["merge-base", owned_commit, frozen_commit],
                frozen_commit,
            ),
            fake_success(
                "jj",
            ...
            fake_success(
                ...
                &[
                    ...
                    "head=stack/owned-title-ownedcha",
                    "-f",
                    "base=stack/dependency-frozen20",
                    "-f",
                    "body=owned body",
                    "--jq",
                    PR_API_JQ,
        ...
        assert!(
            runner
                .calls()
                .iter()
                .any(|call| call.contains("-f base=stack/dependency-frozen20")),
            "owned PR should target the single frozen dependency branch"
        );
    ...
    #[test]
    fn submit_saves_created_pr_cache_before_comment_failure() -> Result<()> {
        let workspace = test_workspace("submit-partial-comment-failure")?;
        let root_parent = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/change-title-changeon";
        let runner = FakeRunner::new(submit_one_change_outputs(
            &workspace,
            root_parent,
            commit,
            "changeone",
            ...
            [
                ...
                fake_success("git", &["ls-remote", "--heads", DEFAULT_REMOTE, branch], ""),
                fake_success(
                    "jj",
                    &["bookmark", "set", "--allow-backwards", branch, "-r", commit],
                    "",
                ),
                fake_success(
                    "jj",
                ...
                fake_comment_list_failure(9, "comments unavailable"),
            ],
        ));
        let cli = Cli::try_parse_from(["jj-stack", "submit"])?;
        ...
        let error = run(cli, &runner)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("comment failure should fail submit"));

        assert!(error.contains("comments unavailable"), "{error}");
        let store = CacheStore::load(
            workspace
                .join(".jj")
                .join("repo")
        ...
        let entry = store
            .get_pr("owner/repo", "changeone")
            .unwrap_or_else(|| panic!("created PR cache should be saved before comments"));
        assert_eq!(entry.pr_number, 9);
        assert_eq!(entry.head_branch, branch);
        assert_eq!(entry.stack_comment_id, None);
        assert_eq!(entry.head_sha, commit);
    ...
    #[test]
    fn submit_recovers_after_comment_failure_without_cache_contents() -> Result<()> {
        let workspace = test_workspace("submit-partial-recovery-no-cache")?;
        let root_parent = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/change-title-changeon";
        let first_runner = FakeRunner::new(submit_one_change_outputs(
            &workspace,
            root_parent,
            commit,
            "changeone",
        ...
        let error = run(cli, &first_runner)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("comment failure should fail submit"));
        assert!(error.contains("comments unavailable"), "{error}");
        ...
        let mut change = test_change("changeone", commit, &[root_parent]);
        change.title = "change title".to_owned();
        change.body = "change body".to_owned();
        let expected_body = expected_stack_comment_body(
            &[("changeone", 9, branch, DEFAULT_TRUNK, "change title")],
            "changeone",
        );
        let second_runner = FakeRunner::new(submit_one_change_outputs(
            &workspace,
            root_parent,
            commit,
            "changeone",
            ...
            [
                ...
                fake_comment_list(9, ""),
                fake_local_bookmark(branch, commit),
                fake_tracked_remote_bookmark(branch),
                fake_success(
                    "git",
                    &["ls-remote", "--heads", DEFAULT_REMOTE, branch],
                ...
                fake_comment_list(9, ""),
                fake_comment_create(9, &expected_body, 123),
            ],
        ));
        let cli = Cli::try_parse_from(["jj-stack", "submit"])?;

        run(cli, &second_runner)?;

        let calls = second_runner.calls();
        assert!(
            calls
                .iter()
                .any(|call| call.starts_with("gh pr list --repo owner/repo --head")),
            "second submit should rediscover the PR from GitHub, not cache: {calls:?}"
        );
        assert!(
            calls
                .iter()
                .all(|call| !call.starts_with("gh api -X POST repos/owner/repo/pulls")),
            "second submit should not create a duplicate PR: {calls:?}"
        );
        assert!(
            calls
                .iter()
                .any(|call| call.starts_with("gh api -X POST repos/owner/repo/issues/9/comments")),
            "second submit should create the missing stack comment: {calls:?}"
        );
    ...
    #[test]
    fn submit_ignores_malformed_sqlite_cache_and_uses_live_discovery() -> Result<()> {
        let workspace = test_workspace("submit-malformed-cache")?;
        let cache_path = workspace
            .join(".jj")
            .join("repo")
            .join(CONFIG_PREFIX)
        ...
        let cli = Cli::try_parse_from(["jj-stack", "--verbose", "submit"])?;
        ...
        assert!(
            runner
                .calls()
                .iter()
                .any(|call| call.starts_with("gh api -X POST repos/owner/repo/pulls")),
            "submit should create PR using live discovery despite malformed cache: {:?}",
            runner.calls()
        );
        fs::remove_dir_all(workspace)?;
        Ok(())
    ...
    #[test]
    fn submit_continues_when_sqlite_cache_write_fails() -> Result<()> {
        let workspace = test_workspace("submit-cache-write-fails")?;
        fs::write(
            workspace.join(".jj").join("repo").join(CONFIG_PREFIX),
            "not a directory",
        )?;
        let root_parent = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let commit = "1111111111111111111111111111111111111111";
        ...
        assert!(
            runner
                ...
                .iter()
                .any(|call| call.starts_with("gh api -X POST repos/owner/repo/pulls")),
            "submit should not fail after cache write failure: {:?}",
            runner.calls()
        );
        fs::remove_dir_all(workspace)?;
        Ok(())
    ...
    #[test]
    fn submit_updates_existing_pr_with_patch_and_jj_bookmark_push() -> Result<()> {
        let workspace = test_workspace("submit-update")?;
        let root_parent = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let old_commit = "9999999999999999999999999999999999999999";
        let new_commit = "1111111111111111111111111111111111111111";
        let branch = "stack/change-title-changeon";
        write_cache(
            ...
            PrCacheEntry {
                ...
                base_ref: DEFAULT_TRUNK.to_owned(),
                head_sha: old_commit.to_owned(),
                base_sha: root_parent.to_owned(),
                title: "old title".to_owned(),
                body: "old body".to_owned(),
                created_at: "2026-06-03T12:34:56Z".to_owned(),
                stack_comment_id: Some("comment-123".to_owned()),
                ..test_pr_entry(9, branch)
            },
        )?;
        let runner = FakeRunner::new(submit_one_change_outputs(
            &workspace,
            ...
            [
                ...
                fake_comment_create(
                    9,
                    &expected_stack_comment_body(
                        &[("changeone", 9, branch, DEFAULT_TRUNK, "new title")],
                        "changeone",
                    ),
                    123,
                ),
        ...
        assert!(
            calls.iter().any(|call| call
                == "jj bookmark set --allow-backwards stack/change-title-changeon -r 1111111111111111111111111111111111111111"),
            "submit should move the local jj bookmark before pushing: {calls:?}"
        );
        assert!(
            calls
                .iter()
                .any(|call| call
                    == "jj git push --remote origin --bookmark stack/change-title-changeon"),
            "submit should push the tracked jj bookmark: {calls:?}"
        );
        assert!(
            calls
                .iter()
                .any(|call| call.starts_with("gh api -X PATCH repos/owner/repo/pulls/9")),
            "submit should update existing PRs through gh api PATCH: {calls:?}"
        );
    ...
    #[test]
    fn submit_refuses_to_force_push_branch_that_advanced_out_of_band() -> Result<()> {
        let workspace = test_workspace("submit-out-of-band")?;
        let root_parent = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let old_commit = "9999999999999999999999999999999999999999";
        let new_commit = "1111111111111111111111111111111111111111";
        let foreign = "5555555555555555555555555555555555555555";
        let branch = "stack/change-title-changeon";
        write_cache(
            &workspace,
            "owner/repo",
        ...
        let runner = FakeRunner::new(submit_one_change_outputs(
            ...
            [
                ...
                // The remote branch is at a commit we neither target nor recorded.
                fake_success(
                    "git",
                    &["ls-remote", "--heads", DEFAULT_REMOTE, branch],
        ...
        let error = run(cli, &runner)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("out-of-band branch should fail"));

        assert!(error.contains("out-of-band"), "{error}");
        assert!(
            runner
                .calls()
                .iter()
                .all(|call| !call.starts_with("git push ")),
            "must refuse before pushing: {:?}",
            runner.calls()
        );
    ...
    #[test]
    fn noop_submit_skips_push_and_pr_update() -> Result<()> {
        let workspace = test_workspace("submit-noop")?;
        let root_parent = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/change-title-changeon";
        write_cache(
        ...
        assert!(
            calls.iter().all(|call| !call.starts_with("git push ")),
            "no-op submit should not push unchanged refs: {calls:?}"
        );
        assert!(
            calls
                .iter()
                .all(|call| !call.starts_with("gh api -X PATCH ")),
            "no-op submit should not patch unchanged PRs: {calls:?}"
        );
    ...
    #[test]
    fn submit_updates_stack_comment_found_by_scan_when_cache_has_no_comment_id() -> Result<()> {
        let workspace = test_workspace("submit-comment-no-cache-id")?;
        let root_parent = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/change-title-changeon";
        write_cache(
        ...
        let runner = FakeRunner::new(submit_one_change_outputs(
            ...
            [
                ...
                fake_comment_update(555, &expected_body, 555),
            ],
        ));
        let cli = Cli::try_parse_from(["jj-stack", "submit"])?;
        ...
        assert!(
            calls.iter().any(
                |call| call.starts_with("gh api -X PATCH repos/owner/repo/issues/comments/555")
            ),
            "submit should update the scanned comment: {calls:?}"
        );
        assert!(
            calls
                .iter()
                .all(|call| !call.starts_with("gh api -X POST repos/owner/repo/issues/9/comments")),
            "submit should not create a duplicate stack comment: {calls:?}"
        );
        fs::remove_dir_all(workspace)?;
        Ok(())
    }
    ...
    #[test]
    fn submit_updates_stack_comment_found_by_scan_when_cached_comment_id_is_stale() -> Result<()> {
        let workspace = test_workspace("submit-comment-stale-cache-id")?;
        let root_parent = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/change-title-changeon";
        write_cache(
            ...
            PrCacheEntry {
                ...
                body: "change body".to_owned(),
                created_at: "2026-06-03T12:34:56Z".to_owned(),
                stack_comment_id: Some("123".to_owned()),
                ..test_pr_entry(9, branch)
            },
        )?;
        let expected_body = expected_stack_comment_body(
        ...
        assert!(
            ...
            "submit should update the live scanned comment, not cached id 123: {calls:?}"
        );
        assert!(
            calls.iter().all(|call| !call.contains("comments/123")),
            "cached stale comment id must not be used: {calls:?}"
        );
        fs::remove_dir_all(workspace)?;
        Ok(())
    }
    ...
    #[test]
    fn submit_after_unfreeze_does_not_update_foreign_stack_comment() -> Result<()> {
        let workspace = test_workspace("submit-adopted-comment")?;
        let root_parent = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/change-title-changeon";
        write_cache(
        ...
        let runner = FakeRunner::new(submit_one_change_outputs(
            ...
            [
                ...
                fake_comment_create(9, &expected_body, 555),
            ],
        ));
        let cli = Cli::try_parse_from(["jj-stack", "submit"])?;
        ...
        let calls = runner.calls();
        assert!(
            calls.iter().all(|call| !call.contains("comments/444")),
            "submit should not update another user's stack comment: {calls:?}"
        );
        assert!(
            calls
                .iter()
                .any(|call| call.starts_with("gh api -X POST repos/owner/repo/issues/9/comments")),
            "submit should create the adopter's own stack comment: {calls:?}"
        );
        let store = CacheStore::load_current(&runner)?;
        assert_eq!(
            store
                .get_pr("owner/repo", "changeone")
                .and_then(|entry| entry.stack_comment_id.as_deref()),
            Some("555")
        );
    ...
    #[test]
    fn submit_base_validation_fails_before_push() -> Result<()> {
        let workspace = test_workspace("submit-bad-base")?;
        let root_parent = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let wrong_base = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let commit = "1111111111111111111111111111111111111111";
        let runner = FakeRunner::new([
            fake_success(
                "jj",
            ...
            fake_success("gh", &["api", "user", "--jq", ".login"], "octocat"),
            fake_success("git", &["merge-base", commit, DEFAULT_TRUNK], wrong_base),
            fake_success(
                "jj",
                &["root"],
                workspace
        ...
        let error = run(cli, &runner)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("submit should reject wrong merge base"));

        assert!(error.contains("submit base validation failed"), "{error}");
        assert!(
            runner
                .calls()
                .iter()
                .all(|call| !call.starts_with("git push ")),
            "failed base validation must not push refs: {:?}",
            runner.calls()
        );
    ...
    #[test]
    fn dry_run_submit_plans_without_mutating_remote_github_or_cache() -> Result<()> {
        let workspace = test_workspace("submit-dry-run")?;
        let root_parent = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/change-title-changeon";
        let runner = FakeRunner::new(submit_one_change_outputs(
        ...
        let cli = Cli::try_parse_from(["jj-stack", "submit", "--dry-run"])?;
        ...
        assert!(
            calls.iter().all(|call| !call.starts_with("git push ")),
            "dry-run must not push: {calls:?}"
        );
        assert!(
            calls
                .iter()
                .all(|call| !(call.starts_with("gh api -X POST ")
                    || call.starts_with("gh api -X PATCH "))),
            "dry-run must not mutate GitHub: {calls:?}"
        );
        assert!(
            !workspace
                .join(".jj")
                ...
                .join("cache.sqlite")
                .exists(),
            "dry-run must not write local cache"
        );
    ...
    #[test]
    fn sync_with_submit_flag_fetches_moves_trunk_rebases_and_submits() -> Result<()> {
        let workspace = test_workspace("sync-submit")?;
        let old_trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let new_trunk = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/change-title-changeon";
        let runner = FakeRunner::new(sync_one_change_outputs(
            &workspace,
            new_trunk,
            commit,
            "changeone",
            ...
            "change body",
            [
                fake_success("jj", &["git", "fetch", "--remote", DEFAULT_REMOTE], ""),
                fake_jj_remote_trunk(new_trunk),
                fake_success("git", &["rev-parse", DEFAULT_TRUNK], old_trunk),
                fake_success(
                    "git",
                    &["rev-parse", &format!("{DEFAULT_REMOTE}/{DEFAULT_TRUNK}")],
                    new_trunk,
                ),
                fake_success(
                    "git",
                    &["merge-base", "--is-ancestor", old_trunk, new_trunk],
                    "",
                ),
                fake_success(
                    "jj",
                    &["bookmark", "set", DEFAULT_TRUNK, "-r", "main@origin"],
                    "",
                ),
                fake_success("jj", &["rebase", "-s", commit, "-d", DEFAULT_TRUNK], ""),
                fake_success(
                    "gh",
                    &[
                        "pr",
        ...
        let calls = runner.calls();
        let fetch_index = command_index(&calls, "jj git fetch --remote origin");
        let move_index = command_index(&calls, "jj bookmark set main -r main@origin");
        let rebase_index = command_index(&calls, &format!("jj rebase -s {commit} -d main"));
        let push_index = command_index(
            &calls,
            "jj git push --remote origin --bookmark stack/change-title-changeon",
        );
        assert!(
            fetch_index < move_index && move_index < rebase_index && rebase_index < push_index,
            "sync should fetch, move trunk, rebase, then submit: {calls:?}"
        );
    ...
    #[test]
    fn sync_skips_submit_by_default_after_rebase() -> Result<()> {
        let workspace = test_workspace("sync-no-submit")?;
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let commit = "1111111111111111111111111111111111111111";
        let runner = FakeRunner::new(sync_one_change_outputs(
            &workspace,
            ...
            [
                fake_success("jj", &["git", "fetch", "--remote", DEFAULT_REMOTE], ""),
                fake_jj_remote_trunk(trunk),
                fake_success("git", &["rev-parse", DEFAULT_TRUNK], trunk),
                fake_success(
                    "git",
                    &["rev-parse", &format!("{DEFAULT_REMOTE}/{DEFAULT_TRUNK}")],
                    trunk,
        ...
        let cli = Cli::try_parse_from(["jj-stack", "sync"])?;

        run(cli, &runner)?;

        let calls = runner.calls();
        assert!(
            calls.iter().any(|call| call == "jj rebase -s 1111111111111111111111111111111111111111 -d main"),
            "sync should still rebase the stack root: {calls:?}"
        );
        assert!(
            calls.iter().all(|call| !call.starts_with("gh ")),
            "sync without --submit should not touch GitHub: {calls:?}"
        );
        assert!(
            calls.iter().all(|call| !call.starts_with("git push ")),
            "sync --no-submit should not submit branches: {calls:?}"
        );
    ...
    #[test]
    fn sync_rebases_owned_stack_onto_nearest_frozen_dependency() -> Result<()> {
        let workspace = test_workspace("sync-frozen-boundary")?;
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let frozen_commit = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let advanced_frozen_commit = "cccccccccccccccccccccccccccccccccccccccc";
        let owned_commit = "1111111111111111111111111111111111111111";
        let frozen_branch = "stack/frozen-title-frozench";
        let runner = FakeRunner::new(sync_one_change_outputs(
            &workspace,
            frozen_commit,
            owned_commit,
            ...
            [
                ...
                fake_jj_remote_trunk(trunk),
                fake_frozen_bookmarks(
                    "jj-stack/frozen/pr-11\tok\tbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb\n",
                ),
                fake_success(
                    "jj",
                    &[
                ...
                fake_success(
                    "jj",
                    &["rebase", "-s", owned_commit, "-d", "jj-stack/frozen/pr-11"],
                    "",
                ),
            ],
        ));
        ...
        let calls = runner.calls();
        let remote_fetch_index = command_index(&calls, "jj git fetch --remote origin");
        let pr_fetch_index = command_index(
            &calls,
            "gh api repos/owner/repo/pulls/11 --jq {number,state,headRefName:.head.ref,baseRefName:.base.ref,headRefOid:.head.sha,baseRefOid:.base.sha,title,body,createdAt:.created_at,id:.node_id,headRepository:{id:(.head.repo.id|tostring),node_id:.head.repo.node_id,nameWithOwner:.head.repo.full_name},baseRepository:{id:(.base.repo.id|tostring),node_id:.base.repo.node_id,nameWithOwner:.base.repo.full_name},author:{login:.user.login}}",
        );
        let branch_fetch_index = command_index(
            &calls,
            "jj git fetch --remote origin --branch stack/frozen-title-frozench",
        );
        let bookmark_index = command_index(
            &calls,
            "jj bookmark set jj-stack/frozen/pr-11 -r cccccccccccccccccccccccccccccccccccccccc",
        );
        assert!(
            remote_fetch_index < pr_fetch_index && branch_fetch_index < bookmark_index,
            "sync should fetch remote before comparing GitHub and fetch the PR branch before moving the frozen bookmark: {calls:?}"
        );
        assert!(
            calls.iter().any(|call| call
                == "jj rebase -s 1111111111111111111111111111111111111111 -d jj-stack/frozen/pr-11"),
            "sync should rebase owned stack onto nearest frozen dependency: {calls:?}"
        );
        assert!(
            calls.iter().any(|call| call
                == "jj bookmark set jj-stack/frozen/pr-11 -r cccccccccccccccccccccccccccccccccccccccc"),
            "sync should fast-forward the frozen dependency bookmark first: {calls:?}"
        );
        assert!(
            calls.iter().all(|call| call
                != "jj rebase -s 1111111111111111111111111111111111111111 -d main"),
            "sync should not rebase owned stack onto trunk when a frozen dependency is underneath: {calls:?}"
        );
        assert!(
            calls.iter().all(|call| !call.contains("/issues/")),
            "sync should not mutate or scan frozen PR comments: {calls:?}"
        );
    ...
    #[test]
    fn sync_rejects_divergent_collaborator_rewrite() -> Result<()> {
        let workspace = test_workspace("sync-frozen-divergent")?;
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let frozen_commit = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let rewritten_commit = "dddddddddddddddddddddddddddddddddddddddd";
        let owned_commit = "1111111111111111111111111111111111111111";
        let frozen_branch = "stack/frozen-title-frozench";
        let runner = FakeRunner::new(sync_one_change_outputs(
            &workspace,
        ...
        let error = run(cli, &runner)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("divergent collaborator rewrite should fail"));

        assert!(error.contains("divergent collaborator rewrite"), "{error}");
        assert!(error.contains("jj-stack/frozen/pr-11"), "{error}");
        let calls = runner.calls();
        assert!(
            calls.iter().all(|call| {
                call != "jj bookmark set jj-stack/frozen/pr-11 -r dddddddddddddddddddddddddddddddddddddddd"
            }),
            "sync should not move divergent frozen bookmark: {calls:?}"
        );
        assert!(
            calls.iter().all(|call| !call.starts_with("jj rebase ")),
            "sync should stop before rebasing owned work after divergent dependency rewrite: {calls:?}"
        );
    ...
    #[test]
    fn sync_rejects_deleted_branch_for_open_frozen_dependency() -> Result<()> {
        let workspace = test_workspace("sync-frozen-deleted-branch")?;
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let frozen_commit = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let advanced_frozen_commit = "cccccccccccccccccccccccccccccccccccccccc";
        let owned_commit = "1111111111111111111111111111111111111111";
        ...
        let error = run(cli, &runner)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("deleted open branch should fail"));

        assert!(
            error.contains("open PR branch may have been deleted"),
            "{error}"
        );
        assert!(error.contains("stack/frozen-title-frozench"), "{error}");
        let calls = runner.calls();
        assert!(
            calls.iter().all(|call| {
                call != "jj bookmark set jj-stack/frozen/pr-11 -r cccccccccccccccccccccccccccccccccccccccc"
            }),
            "sync should not move frozen bookmark when the open branch cannot be fetched: {calls:?}"
        );
        assert!(
            calls.iter().all(|call| !call.starts_with("jj rebase ")),
            "sync should stop before rebasing owned work when a frozen dependency branch is deleted: {calls:?}"
        );
    ...
    #[test]
    fn sync_recovers_merged_frozen_dependency_by_rebasing_owned_to_trunk() -> Result<()> {
        let workspace = test_workspace("sync-frozen-merged")?;
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let frozen_commit = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let owned_commit = "1111111111111111111111111111111111111111";
        let frozen_branch = "stack/frozen-title-frozench";
        let runner = FakeRunner::new(sync_one_change_outputs(
            ...
            [
                ...
                fake_success(
                    "jj",
                    &["rebase", "-s", owned_commit, "-d", DEFAULT_TRUNK],
                    "",
                ),
            ],
        ));
        ...
        assert!(
            calls
                .iter()
                .any(|call| call == "jj rebase -s 1111111111111111111111111111111111111111 -d main"),
            "merged frozen dependency should make sync rebase owned work onto trunk: {calls:?}"
        );
        assert!(
            calls
                .iter()
                .all(|call| call != "jj rebase -s 1111111111111111111111111111111111111111 -d jj-stack/frozen/pr-11"),
            "merged frozen dependency should no longer be the rebase boundary: {calls:?}"
        );
        assert!(
            calls.iter().all(|call| {
                call != "jj git fetch --remote origin --branch stack/frozen-title-frozench"
            }),
            "merged dependency recovery should not require the merged PR branch to exist: {calls:?}"
        );
        assert!(
            calls.iter().all(|call| {
                call != "git merge-base --is-ancestor bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
            }),
            "squash-merged dependency recovery must not require the frozen head to be an ancestor of trunk: {calls:?}"
        );
    ...
    #[test]
    fn sync_supports_purely_frozen_imported_stack_at_current_revision() -> Result<()> {
        let workspace = test_workspace("sync-purely-frozen")?;
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let frozen_commit = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let frozen_branch = "stack/frozen-title-frozench";
        let runner = FakeRunner::new([
            ...
            // Merged-branch cleanup lists local stack bookmarks; none here.
            fake_success("jj", &["bookmark", "list", "-T", LOCAL_BOOKMARK_TEMPLATE], ""),
        ]);
        let cli = Cli::try_parse_from(["jj-stack", "sync"])?;
        ...
        assert!(
            calls.iter().any(|call| {
                call == "jj bookmark set jj-stack/frozen/pr-11 -r bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
            }),
            "purely frozen sync should refresh the current frozen stack: {calls:?}"
        );
        assert!(
            calls.iter().all(|call| !call.starts_with("jj rebase ")),
            "purely frozen sync should not rebase without owned changes: {calls:?}"
        );
        assert!(
            calls.iter().all(|call| {
                !call.starts_with("gh api -X POST") && !call.starts_with("gh api -X PATCH")
            }),
            "purely frozen sync should not submit without owned changes: {calls:?}"
        );
    ...
    #[test]
    fn sync_stops_before_submit_when_rebase_leaves_conflicts() -> Result<()> {
        let workspace = test_workspace("sync-rebase-conflict")?;
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let commit = "1111111111111111111111111111111111111111";
        let runner = FakeRunner::new(sync_one_change_outputs(
            &workspace,
        ...
        let error = run(cli, &runner)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("conflicted post-rebase stack should fail"));

        assert!(error.contains("conflicted change"), "{error}");
        let calls = runner.calls();
        assert!(
            calls.iter().all(|call| !call.starts_with("gh ")),
            "sync should stop before submit/GitHub after rebase conflicts: {calls:?}"
        );
    ...
    #[test]
    fn dry_run_sync_skips_fetch_trunk_move_rebase_and_submit_mutations() -> Result<()> {
        let workspace = test_workspace("sync-dry-run")?;
        let old_trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let new_trunk = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let commit = "1111111111111111111111111111111111111111";
        let runner = FakeRunner::new(sync_one_change_outputs(
        ...
        let cli = Cli::try_parse_from(["jj-stack", "sync", "--dry-run"])?;
        ...
        for forbidden in [
            ...
            assert!(
                calls.iter().all(|call| !call.starts_with(forbidden)),
                "dry-run sync should not run mutation `{forbidden}`: {calls:?}"
            );
        }
    ...
    #[test]
    fn sync_rejects_diverged_trunk_before_rebase_and_prints_both_commits() -> Result<()> {
        let workspace = test_workspace("sync-diverged")?;
        let local_trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let remote_trunk = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let commit = "1111111111111111111111111111111111111111";
        let runner = FakeRunner::new(sync_one_change_outputs(
            &workspace,
            local_trunk,
            ...
            [
                fake_success("jj", &["git", "fetch", "--remote", DEFAULT_REMOTE], ""),
                fake_jj_remote_trunk(remote_trunk),
                fake_success("git", &["rev-parse", DEFAULT_TRUNK], local_trunk),
                fake_success(
                    "git",
                    &["rev-parse", &format!("{DEFAULT_REMOTE}/{DEFAULT_TRUNK}")],
                    remote_trunk,
                ...
                fake_failure(
                    "git",
                    &["merge-base", "--is-ancestor", local_trunk, remote_trunk],
                    "not ancestor",
                ),
            ],
        ));
        ...
        let error = run(cli, &runner)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("diverged trunk should fail"));

        assert!(error.contains(local_trunk), "{error}");
        assert!(error.contains(remote_trunk), "{error}");
        assert!(
            runner
                .calls()
                .iter()
                .all(|call| !call.starts_with("jj rebase ")),
            "diverged trunk must fail before rebasing: {:?}",
            runner.calls()
        );
    ...
    #[test]
    fn merge_frozen_preflight_blocks_open_dependency() -> Result<()> {
        let frozen_commit = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let owned_commit = "1111111111111111111111111111111111111111";
        let context = AppContext::new(
            test_github(),
            StackResolution {
                owned: vec![test_change("ownedchange", owned_commit, &[frozen_commit])],
                frozen_dependencies: vec![test_frozen_dependency(
                    20,
                    frozen_commit,
                    &["aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"],
                )],
        ...
        let bottom_owned_pr: GhMergePr = serde_json::from_str(&merge_pr_json(
            22,
            "OPEN",
            "stack/owned-title-ownedcha",
            "stack/dependency-frozen20",
            owned_commit,
            frozen_commit,
            "CLEAN",
        ))?;
        let runner = FakeRunner::new([fake_merge_pr_view(
            20,
            &merge_pr_json(
                20,
                "OPEN",
        ...
        let error =
            validate_merge_frozen_dependencies(&runner, &test_config(), &context, &bottom_owned_pr)
                .err()
                .map(|error| error.to_string())
                .unwrap_or_else(|| panic!("open frozen dependency should block merge"));

        assert!(error.contains("still open"), "{error}");
        assert!(error.contains("jj-stack sync"), "{error}");
        Ok(())
    }

    #[test]
    fn merge_frozen_preflight_requires_sync_after_dependency_merged() -> Result<()> {
        let frozen_commit = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let owned_commit = "1111111111111111111111111111111111111111";
        let context = AppContext::new(
            test_github(),
        ...
        let error =
            validate_merge_frozen_dependencies(&runner, &test_config(), &context, &bottom_owned_pr)
                .err()
                .map(|error| error.to_string())
                .unwrap_or_else(|| panic!("stale owned base should require sync"));

        assert!(error.contains("still targets"), "{error}");
        assert!(error.contains("jj-stack sync"), "{error}");
        Ok(())
    }

    #[test]
    fn merge_frozen_preflight_allows_synced_owned_pr_after_dependency_merged() -> Result<()> {
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let frozen_commit = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let owned_commit = "1111111111111111111111111111111111111111";
        let context = AppContext::new(
            test_github(),
            StackResolution {
                owned: vec![test_change("ownedchange", owned_commit, &[trunk])],
                frozen_dependencies: vec![test_frozen_dependency(20, frozen_commit, &[trunk])],
            },
        );
        let bottom_owned_pr: GhMergePr = serde_json::from_str(&merge_pr_json(
            22,
    ...
    #[test]
    fn status_json_reports_simple_owned_stack() -> Result<()> {
        let workspace = test_workspace("status-simple")?;
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/change-title-changeon";
        let runner = FakeRunner::new([
        ...
        assert_eq!(json["remote"], DEFAULT_REMOTE);
        assert_eq!(json["trunk"], DEFAULT_TRUNK);
        assert_eq!(json["branch_prefix"], DEFAULT_BRANCH_PREFIX);
        assert_eq!(json["owned_prs"][0]["head_branch"], branch);
        assert_eq!(json["owned_prs"][0]["base_branch"], DEFAULT_TRUNK);
        assert_eq!(json["owned_prs"][0]["action"], "create");
        assert_eq!(json["first_owned_base_branch"], DEFAULT_TRUNK);
        assert_eq!(json["suggested_next_command"], "jj-stack submit");
        assert!(json["startup_aliases"]["immutable_heads"].is_string());
    ...
    #[test]
    fn status_json_reports_frozen_dependency_and_stale_problem() -> Result<()> {
        let workspace = test_workspace("status-frozen")?;
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let frozen_commit = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let remote_frozen_commit = "cccccccccccccccccccccccccccccccccccccccc";
        let owned_commit = "1111111111111111111111111111111111111111";
        let owned_branch = "stack/change-title-changeon";
        let frozen_branch = "stack/dependency-frozen20";
        let runner = FakeRunner::new([
            fake_success(
                "jj",
            ...
            fake_success(
                "git",
                &["show", "-s", "--format=%T", owned_commit],
                "ownedtree",
            ),
            fake_frozen_bookmarks(&format!("jj-stack/frozen/pr-20\tok\t{frozen_commit}\n")),
            fake_success(
                "jj",
                &[
                    "log",
        ...
        assert_eq!(
            json["frozen_dependencies"][0]["bookmark"],
            "jj-stack/frozen/pr-20"
        );
        assert_eq!(json["frozen_dependencies"][0]["head_branch"], frozen_branch);
        assert!(
            json["frozen_dependencies"][0]["problem"]
                .as_str()
                .unwrap_or_default()
                .contains("stale")
        );
        assert_eq!(json["first_owned_base_branch"], frozen_branch);
        assert_eq!(json["suggested_next_command"], "jj-stack sync");
    ...
    #[test]
    fn status_json_reports_untracked_own_bookmark_problem() -> Result<()> {
        let workspace = test_workspace("status-bookmark-problem")?;
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/change-title-changeon";
        write_cache(
        ...
        let runner = FakeRunner::new([
            ...
            fake_local_stack_bookmarks(&test_change("changeone", commit, &[trunk]), [branch]),
            fake_success(
                "gh",
                &[
                    "pr",
            ...
            fake_comment_list(9, ""),
            fake_local_bookmark(branch, commit),
            fake_remote_bookmark_status(branch, false, false),
        ]);
        ...
        assert!(
            json["bookmark_problems"][0]
                .as_str()
                .unwrap_or_default()
                .contains("untracked"),
            "{json:#}"
        );
        assert_eq!(json["owned_prs"][0]["action"], "blocked");
    ...
    #[test]
    fn status_json_reports_merge_blockers() -> Result<()> {
        let workspace = test_workspace("status-merge-blocker")?;
        let trunk = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let commit = "1111111111111111111111111111111111111111";
        let branch = "stack/change-title-changeon";
        write_cache(
        ...
        assert!(
            json["merge_blockers"][0]
                .as_str()
                .unwrap_or_default()
                .contains("draft")
        );
        assert_eq!(json["owned_prs"][0]["pr_number"], 9);
        assert_eq!(json["owned_prs"][0]["action"], "unchanged");
    ...
    #[test]
    fn stack_comment_body_uses_compact_pr_links() {
        let context = AppContext::from_owned(
            test_github(),
            vec![
                test_change(
                    "bottomchange",
        ...
        let entries = vec![
            (
                "bottomchange".to_owned(),
                test_pr_entry(11, "stack/bottom-title-bottomch"),
            ),
            (
                "topchange".to_owned(),
                test_pr_entry(12, "stack/top-title-topchang"),
            ),
        ];

        assert_eq!(
            stack_comment_body(&context, &entries, "topchange", DEFAULT_TRUNK),
            concat!(
                "<!-- stack:v1 -->\n",
                "Stack for owner/repo\n\n",
                "- **[topchange title #12](https://github.com/owner/repo/pull/12)** _topchang_ · 2026-06-03 12:34:56 👈\n",
                "- [bottomchange title #11](https://github.com/owner/repo/pull/11) _bottomch_ · 2026-06-03 12:34:56\n",
                "- main\n",
                "\n",
                "Check out this stack: `jj-stack get https://github.com/owner/repo/pull/12`\n",
                "Pull/update this stack: `jj-stack sync`\n",
                "Publish local edits: `jj-stack submit`\n",
                "Merge when ready: `jj-stack merge`\n"
            )
    ...
    #[test]
    fn markdown_link_label_escapes_link_syntax_and_control_chars() {
        assert_eq!(
    ...
    #[test]
    fn status_checks_block_on_failing_pending_and_pass_on_success() {
        // GraphQL `state` shape: a failing legacy status context blocks.
        let failing = serde_json::json!([{"context": "ci", "state": "FAILURE"}]);
        let error = validate_status_checks(7, failing.as_array().unwrap())
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("failing check should block"));
        assert!(error.contains("checks are not passing"), "{error}");
        assert!(
            !error.contains("required checks"),
            "wording should not claim required: {error}"
        );

        // CheckRun shape: an incomplete run is pending.
        let pending = serde_json::json!([{"name": "build", "status": "IN_PROGRESS"}]);
        let error = validate_status_checks(7, pending.as_array().unwrap())
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("pending check should block"));
        assert!(error.contains("checks are pending"), "{error}");

        // Completed runs that succeeded, were skipped, or neutral all pass.
        let passing = serde_json::json!([
            {"context": "ci", "state": "SUCCESS"},
            {"name": "lint", "status": "COMPLETED", "conclusion": "SKIPPED"},
        ]);
        assert!(validate_status_checks(7, passing.as_array().unwrap()).is_ok());
    }

    #[test]
    fn merge_preflight_blocks_draft_unapproved_and_unmergeable_prs() {
        let config = test_config();
        let change = test_change("changeone", "headsha", &["0".repeat(40).as_str()]);
        let entry = test_pr_entry(9, "stack/change-title-changeon");

        for (field, value, expected) in [
            ("isDraft", serde_json::json!(true), "is draft"),
            (
                "reviewDecision",
            ...
            let mut pr_json = serde_json::from_str::<serde_json::Value>(&merge_pr_json(
                9,
                "OPEN",
                "stack/change-title-changeon",
                DEFAULT_TRUNK,
            ...
            .unwrap_or_else(|error| panic!("parse merge pr json: {error}"));
            pr_json[field] = value;
            let pr = serde_json::from_value::<GhMergePr>(pr_json)
                .unwrap_or_else(|error| panic!("parse merge pr: {error}"));

            let error =
                validate_pr_ready_for_merge(&config, &change, &entry, &pr, false)
                    .err()
                    .map(|error| error.to_string())
                    .unwrap_or_else(|| panic!("{field} should block merge"));

            assert!(
                error.contains(expected),
                "{field} should produce `{expected}` error, got {error}"
            );
        }
    }

    #[test]
    fn move_trunk_bails_when_git_ref_disagrees_with_jj_remote_bookmark() {
        let stale_git = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
        let fresh_jj = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let runner = FakeRunner::new([
            fake_success("git", &["rev-parse", DEFAULT_TRUNK], stale_git),
            fake_success(
                "git",
                &["rev-parse", &format!("{DEFAULT_REMOTE}/{DEFAULT_TRUNK}")],
                stale_git,
            ),
            // jj sees a fresher remote bookmark than the colocated git ref.
            fake_jj_remote_trunk(fresh_jj),
        ]);
        let config = test_config();
        let diagnostics = Diagnostics {
            verbose: false,
        ...
        let error = move_trunk_to_remote(&runner, &config, diagnostics)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("stale git ref should fail trunk movement"));

        assert!(error.contains("remote trunk views disagree"), "{error}");
        assert!(
            runner
                .calls()
                .iter()
                .all(|call| !call.starts_with("jj bookmark set")),
            "must not move trunk on drift: {:?}",
            runner.calls()
        );
    }

    #[test]
    fn fetch_remote_fails_when_remote_trunk_bookmark_is_unresolvable() {
        let runner = FakeRunner::new([
            fake_success("jj", &["git", "fetch", "--remote", DEFAULT_REMOTE], ""),
            fake_failure(
                "jj",
        ...
        let error = fetch_remote(&runner, &config, diagnostics)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("unresolvable remote trunk should fail the fetch gate"));

        assert!(error.contains("not resolvable"), "{error}");
    }

    #[test]
    fn merge_rejects_pr_whose_base_is_not_trunk() {
        let config = test_config();
        let change = test_change("changeone", "headsha", &["0".repeat(40).as_str()]);
        let entry = test_pr_entry(9, "stack/change-title-changeon");
        let pr: GhMergePr = serde_json::from_str(&merge_pr_json(
            9,
            "OPEN",
            "stack/change-title-changeon",
            "stack/other-title",
        ...
        let error = validate_pr_ready_for_merge(&config, &change, &entry, &pr, false)
            .err()
            .map(|error| error.to_string())
            .unwrap_or_else(|| panic!("merge with wrong base should fail"));

        assert!(error.contains("must target trunk"), "{error}");
        assert!(error.contains("stack/other-title"), "{error}");
    }

    #[test]
    fn repoint_pr_base_patches_pr_base_to_trunk() -> Result<()> {
        let runner = FakeRunner::new([fake_success(
            "gh",
            &[
                "api",
        ...
        repoint_pr_base(&runner, &test_github(), 9, "main", Diagnostics::default())?;
        assert!(
            runner
                .calls()
                .iter()
                .any(|c| c == "gh api -X PATCH repos/owner/repo/pulls/9 -f base=main"),
            "{:?}",
            runner.calls()
        );
        Ok(())
    ...
    #[test]
    fn fast_forward_trunk_over_stack_sets_bookmark_and_pushes_once() -> Result<()> {
        let remote = "a".repeat(40);
        let top = "b".repeat(40);
        let runner = FakeRunner::new([
            fake_success("git", &["rev-parse", "origin/main"], &remote),
            fake_success(
                "git",
                &["merge-base", "--is-ancestor", remote.as_str(), top.as_str()],
                "",
            ),
            fake_success("jj", &["bookmark", "set", "main", "-r", top.as_str()], ""),
            fake_success(
                "jj",
        ...
        fast_forward_trunk_over_stack(&runner, &test_config(), &top, Diagnostics::default())?;
        let calls = runner.calls();
        assert!(
            calls.iter().any(|c| *c == format!("jj bookmark set main -r {top}")),
            "{calls:?}"
        );
        assert!(
            calls
                .iter()
                .any(|c| c == "jj git push --remote origin --bookmark main"),
            "{calls:?}"
        );
        Ok(())
    }
    ...
    #[test]
    fn fast_forward_trunk_over_stack_refuses_non_fast_forward() {
        let remote = "a".repeat(40);
        let top = "b".repeat(40);
        let runner = FakeRunner::new([
            fake_success("git", &["rev-parse", "origin/main"], &remote),
        ...
        let error =
            fast_forward_trunk_over_stack(&runner, &test_config(), &top, Diagnostics::default())
                .err()
                .map(|e| e.to_string())
                .unwrap_or_else(|| panic!("non-fast-forward trunk should fail"));
        assert!(error.contains("cannot fast-forward"), "{error}");
        assert!(
            runner.calls().iter().all(|c| !c.starts_with("jj git push")),
            "must not push on non-fast-forward: {:?}",
            runner.calls()
        );
    }

    #[test]
    fn verify_prs_merged_succeeds_when_all_prs_merged() -> Result<()> {
        let runner = FakeRunner::new([fake_success(
            "gh",
            &[
                "pr", "view", "9", "--repo", "owner/repo", "--json", "state", "--jq", ".state",
            ],
            "MERGED\n",
        )]);
        verify_prs_merged(&runner, &test_github(), &[9], Diagnostics::default())?;
        Ok(())
    }

    #[test]
    fn settle_candidates_mergeability_polls_until_known() -> Result<()> {
        // First read returns the transient `UNKNOWN` GitHub reports while it
        // recomputes mergeability; the second read has settled to `MERGEABLE`.
        let unknown = merge_pr_json(5, "OPEN", "feat", "main", "h", "b", "CLEAN")
            .replace("\"mergeable\":\"MERGEABLE\"", "\"mergeable\":\"UNKNOWN\"");
        let settled = merge_pr_json(5, "OPEN", "feat", "main", "h", "b", "CLEAN");
        let runner = FakeRunner::new([
            fake_merge_pr_view(5, &unknown),
            fake_merge_pr_view(5, &settled),
        ]);
        let change = test_change("ch", "h", &["main"]);
        let pr: GhMergePr = serde_json::from_str(&unknown)?;
        let mut candidates = vec![MergeCandidate {
            change: &change,
            entry: test_pr_entry(5, "feat"),
            pr,
            needs_settle: true,
        }];
        settle_candidates_mergeability(
            &runner,
            &test_github(),
            &mut candidates,
            Diagnostics::default(),
        )?;
        assert_eq!(candidates[0].pr.mergeable.as_deref(), Some("MERGEABLE"));
        // It must have re-fetched at least twice to get past the `UNKNOWN`.
        let views = runner
            .calls()
            .iter()
            .filter(|c| c.starts_with("gh pr view 5"))
            .count();
        assert!(views >= 2, "expected a re-fetch after UNKNOWN, got {views}");
        Ok(())
    }

    #[test]
    fn settle_candidates_mergeability_settles_multiple_with_shared_rounds() -> Result<()> {
        // Two re-pointed PRs settle together: each is UNKNOWN on the first read
        // and MERGEABLE on the second. They share poll rounds rather than each
        // waiting in series.
        let unknown5 = merge_pr_json(5, "OPEN", "a", "main", "h5", "b", "CLEAN")
            .replace("\"mergeable\":\"MERGEABLE\"", "\"mergeable\":\"UNKNOWN\"");
        let unknown6 = merge_pr_json(6, "OPEN", "b", "main", "h6", "b", "CLEAN")
            .replace("\"mergeable\":\"MERGEABLE\"", "\"mergeable\":\"UNKNOWN\"");
        let settled5 = merge_pr_json(5, "OPEN", "a", "main", "h5", "b", "CLEAN");
        let settled6 = merge_pr_json(6, "OPEN", "b", "main", "h6", "b", "CLEAN");
        let runner = FakeRunner::new([
            fake_merge_pr_view(5, &unknown5),
            fake_merge_pr_view(5, &settled5),
            fake_merge_pr_view(6, &unknown6),
            fake_merge_pr_view(6, &settled6),
        ]);
        let change5 = test_change("c5", "h5", &["main"]);
        let change6 = test_change("c6", "h6", &["main"]);
        let pr5: GhMergePr = serde_json::from_str(&unknown5)?;
        let pr6: GhMergePr = serde_json::from_str(&unknown6)?;
        let mut candidates = vec![
            MergeCandidate {
                change: &change5,
                entry: test_pr_entry(5, "a"),
                pr: pr5,
                needs_settle: true,
            },
            MergeCandidate {
                change: &change6,
                entry: test_pr_entry(6, "b"),
                pr: pr6,
                needs_settle: true,
            },
        ];
        ...
        assert_eq!(candidates[0].pr.mergeable.as_deref(), Some("MERGEABLE"));
        assert_eq!(candidates[1].pr.mergeable.as_deref(), Some("MERGEABLE"));
        Ok(())
    }

    #[test]
    fn pr_is_merged_is_false_for_open_pr() -> Result<()> {
        let runner = FakeRunner::new([fake_success(
            "gh",
            &[
                "pr", "view", "9", "--repo", "owner/repo", "--json", "state", "--jq", ".state",
        ...
        assert!(!pr_is_merged(&runner, &test_github(), 9)?);
        Ok(())
    }

    #[test]
    fn cleanup_merged_branch_deletes_local_and_remote_when_no_open_pr() -> Result<()> {
        let branch = "stack/merged-branch";
        let runner = FakeRunner::new([
            fake_success(
                "gh",
                &[
                    "pr", "list", "--repo", "owner/repo", "--base", branch, "--state", "open",
                    "--json", "number",
                ],
                "[]",
            ),
            fake_success("jj", &["bookmark", "delete", branch], ""),
            fake_success(
                "jj",
        ...
        let cleaned = cleanup_merged_branches(
            &runner,
            &test_config(),
            &test_github(),
            &[branch.to_owned()],
        ...
        assert_eq!(cleaned, 1, "branch with no open dependent PR should be cleaned");
        let calls = runner.calls();
        assert!(
            calls.iter().any(|c| c == &format!("jj bookmark delete {branch}")),
            "{calls:?}"
        );
        assert!(
            calls
                .iter()
                .any(|c| c == &format!("jj git push --remote origin --bookmark {branch}")),
            "deletion must be pushed to the remote: {calls:?}"
        );
        // After the push, the tracking ref is reconciled so a remote that GitHub
        // already auto-deleted doesn't leave a dangling `@origin` bookmark.
        assert!(
            calls
                .iter()
                .any(|c| c == &format!("jj bookmark forget --include-remotes {branch}")),
            "deletion must reconcile the remote-tracking ref: {calls:?}"
        );
        Ok(())
    }

    #[test]
    fn cleanup_merged_branches_pushes_all_deletions_in_one_call() -> Result<()> {
        let a = "stack/merged-a";
        let b = "stack/merged-b";
        let no_open = |branch: &str| {
            fake_success(
                "gh",
                &[
                    "pr", "list", "--repo", "owner/repo", "--base", branch, "--state", "open",
        ...
        let runner = FakeRunner::new([
            no_open(a),
            no_open(b),
            fake_success("jj", &["bookmark", "delete", a], ""),
            fake_success("jj", &["bookmark", "delete", b], ""),
            fake_success(
                "jj",
                &[
                    "git", "push", "--remote", "origin", "--bookmark", a, "--bookmark", b,
                ],
                "",
            ),
        ]);
        ...
        assert_eq!(cleaned, 2);
        let calls = runner.calls();
        // Both bookmarks deleted, but pushed in a SINGLE batched invocation.
        let pushes = calls
            .iter()
            .filter(|c| c.starts_with("jj git push"))
            .count();
        assert_eq!(pushes, 1, "deletions must be pushed in one call: {calls:?}");
        assert!(
            calls
                .iter()
                .any(|c| c == "jj git push --remote origin --bookmark stack/merged-a --bookmark stack/merged-b"),
            "{calls:?}"
        );
        Ok(())
    }
    ...
    #[test]
    fn cleanup_merged_branch_refuses_when_open_pr_bases_on_it() -> Result<()> {
        let branch = "stack/anchor-branch";
        // An open PR still bases on this branch; deleting it would cascade-close
        // that PR, so cleanup must refuse and touch nothing.
        let runner = FakeRunner::new([fake_success(
            "gh",
            &[
                "pr", "list", "--repo", "owner/repo", "--base", branch, "--state", "open",
        ...
        assert_eq!(cleaned, 0, "must not delete a branch an open PR depends on");
        let calls = runner.calls();
        assert!(
            calls.iter().all(|c| !c.starts_with("jj bookmark delete")),
            "must not delete the local bookmark: {calls:?}"
        );
        assert!(
            calls.iter().all(|c| !c.contains("git push")),
            "must not push a deletion: {calls:?}"
        );
        Ok(())
    }

    #[test]
    fn cleanup_landed_branches_cleans_only_ancestors_of_trunk() -> Result<()> {
        let landed = "stack/landed";
        let active = "stack/active";
        let trunk_tip = "t".repeat(40);
        let landed_commit = "1".repeat(40);
        let active_commit = "2".repeat(40);
        let runner = FakeRunner::new([
            fake_success(
                "jj",
                &["bookmark", "list", "-T", LOCAL_BOOKMARK_TEMPLATE],
                &format!("{landed}\t\n{active}\t\n"),
            ),
            fake_success("git", &["rev-parse", "origin/main"], &trunk_tip),
            fake_success(
                "jj",
                &["log", "--no-graph", "-r", landed, "-T", "commit_id"],
                &landed_commit,
            ),
            fake_success(
                "jj",
                &["log", "--no-graph", "-r", active, "-T", "commit_id"],
                &active_commit,
            ),
            // `landed` is in trunk; `active` is not.
            fake_success(
                "git",
                &["merge-base", "--is-ancestor", &landed_commit, &trunk_tip],
                "",
            ),
            fake_failure(
                "git",
                &["merge-base", "--is-ancestor", &active_commit, &trunk_tip],
                "",
            ),
            fake_success(
                "gh",
            ...
            fake_success(
                "gh",
                &[
                    "pr", "list", "--repo", "owner/repo", "--base", landed, "--state", "open",
                    "--json", "number",
                ],
                "[]",
            ),
            fake_success("jj", &["bookmark", "delete", landed], ""),
            fake_success(
                "jj",
                &["git", "push", "--remote", "origin", "--bookmark", landed],
                "",
            ),
        ]);
        let cleaned = cleanup_landed_branches(&runner, &test_config(), Diagnostics::default())?;
        assert_eq!(cleaned, 1, "only the landed branch should be cleaned");
        let calls = runner.calls();
        assert!(
            calls.iter().any(|c| c == &format!("jj bookmark delete {landed}")),
            "{calls:?}"
        );
        assert!(
            calls.iter().all(|c| c != &format!("jj bookmark delete {active}")),
            "the unmerged branch must be preserved: {calls:?}"
        );
        Ok(())
    }

    #[test]
    fn duplicate_stack_comments_update_newest_matching_comment() -> Result<()> {
        let github = test_github();
        let body = format!("{STACK_COMMENT_MARKER}\nbody");
        let runner = FakeRunner::new([
            fake_comment_list(
                ...
                &format!(
                    "{}\n{}",
                    comment_line(111, &body, "octocat", "2026-06-03T17:00:00Z"),
                    comment_line(222, &body, "octocat", "2026-06-03T18:00:00Z")
                ),
            ),
            fake_comment_update(222, &body, 222),
        ]);

        let action = upsert_stack_comment(
            &runner,
            &github,
            9,
            "changeone",
        ...
        assert_eq!(action, StackCommentAction::Updated("222".to_owned(), 1));
        assert!(
            runner.calls().iter().any(
                |call| call.starts_with("gh api -X PATCH repos/owner/repo/issues/comments/222")
            ),
            "duplicate handling should update newest stack comment: {:?}",
            runner.calls()
        );
        Ok(())
    }
    ...
    #[test]
    fn submit_mutation_failures_include_phase_object_command_and_safe_next_command() -> Result<()> {
        let config = test_config();
        let plan = SubmitPlan {
            change: test_change(
                "changeone",
            ...
            head_branch: "stack/change-title-changeon".to_owned(),
            base_branch: DEFAULT_TRUNK.to_owned(),
            existing_pr: None,
        ...
        let message = push_changed_heads(
            &runner,
            &config,
            &[plan],
            Diagnostics {
        ...
        .err()
        .map(|error| error.to_string())
        .unwrap_or_else(|| panic!("push should fail"));

        for expected in [
            "phase=push-refs",
            "object=bookmark:stack/change-title-changeon",
            "failed-command=`jj git push",
            "safe-next-command=`jj-stack submit --dry-run`",
        ] {
    ...
    impl FakeRunner {
        fn new(outputs: impl IntoIterator<Item = (String, CommandOutput)>) -> Self {
            let mut grouped = HashMap::<String, VecDeque<CommandOutput>>::new();
            for (key, output) in outputs {
                grouped.entry(key).or_default().push_back(output);
            }

            Self {
                outputs: RefCell::new(grouped),
        ...
        fn unused_configured_commands(&self) -> Vec<String> {
    ...
    impl CommandRunner for FakeRunner {
        fn run(&self, program: &str, args: &[&str]) -> Result<CommandOutput> {
            ...
            let output = self.outputs.borrow_mut().get_mut(&key).and_then(|outputs| {
                if outputs.len() > 1 {
                    outputs.pop_front()
                } else {
            ...
            Ok(output.unwrap_or_else(|| default_fake_output(program, args)))
        }
    }

    fn default_fake_output(program: &str, args: &[&str]) -> CommandOutput {
        if program == "jj" && args == ["config", "get", JJ_CONFIG_IMMUTABLE_ALIAS_KEY] {
            return CommandOutput {
                success: true,
                stdout: format!("{JJ_DEFAULT_IMMUTABLE_ALIAS_VALUE}\n"),
                stderr: String::new(),
            };
        }

        if program == "jj"
            && (args == ["config", "get", JJ_CONFIG_FROZEN_ALIAS_KEY]
                || args == ["config", "get", JJ_CONFIG_BASE_IMMUTABLE_ALIAS_KEY])
        {
            return CommandOutput {
                success: false,
                stdout: String::new(),
                stderr: "not configured".to_owned(),
            };
        }

        if program == "jj" && args.starts_with(&["config", "set", "--repo"]) {
            return CommandOutput {
                success: true,
                stdout: String::new(),
                stderr: String::new(),
        ...
        if program == "jj"
            && args.starts_with(&["bookmark", "list", "--revision"])
            && args.contains(&LOCAL_BOOKMARK_TEMPLATE)
        {
            return CommandOutput {
                success: true,
                stdout: String::new(),
    ...
    fn fake_success(program: &str, args: &[&str], stdout: &str) -> (String, CommandOutput) {
        (
    ...
    fn fake_failure(program: &str, args: &[&str], stderr: &str) -> (String, CommandOutput) {
        (
            command_key(program, args),
            CommandOutput {
                success: false,
    ...
    fn assert_rejects_shape(stack: &[ResolvedChange], expected: &str) {
        let result = validate_stack_shape(stack, DEFAULT_STACK_REVSET);
        assert!(result.is_err(), "stack shape should be rejected");
        ...
        assert!(
            message.contains("--revset"),
            "expected error to suggest --revset, got `{message}`"
        );
    }

    fn linear_stack() -> Vec<ResolvedChange> {
        let bottom_commit = "1111111111111111111111111111111111111111";
        vec![
            test_change(
                "bottomchange",
    ...
    fn dependency_pr_numbers(dependencies: &[FrozenDependency]) -> Vec<u64> {
        dependencies
            .iter()
            .map(|dependency| dependency.bookmark.pr_number)
            .collect()
    }

    fn test_frozen_dependency(
        pr_number: u64,
        commit_id: &str,
        parent_ids: &[&str],
    ) -> FrozenDependency {
        FrozenDependency {
            bookmark: FrozenBookmark {
                name: format!("{FROZEN_BOOKMARK_PREFIX}{pr_number}"),
                pr_number,
                commit_id: commit_id.to_owned(),
            },
            change: test_change(&format!("frozen{pr_number}"), commit_id, parent_ids),
        }
    }

    fn test_change(change_id: &str, commit_id: &str, parent_ids: &[&str]) -> ResolvedChange {
        ResolvedChange {
            ...
            parent_ids: parent_ids
                ...
                .collect(),
            title: format!("{change_id} title"),
            body: String::new(),
            tree_id: format!("{change_id}tree"),
            empty: false,
            conflict: false,
        }
    }

    fn test_github() -> GitHubContext {
        GitHubContext {
            repo: "owner/repo".to_owned(),
            username: "octocat".to_owned(),
        }
    ...
    fn test_workspace(name: &str) -> Result<PathBuf> {
        let workspace = unique_test_dir(name);
        fs::create_dir_all(workspace.join(".jj").join("repo"))?;
        Ok(workspace)
    }
    ...
    fn sync_one_change_outputs(
        ...
        title: &str,
        body: &str,
        extra: impl IntoIterator<Item = (String, CommandOutput)>,
    ) -> Vec<(String, CommandOutput)> {
        let mut outputs =
            submit_one_change_outputs(workspace, root_parent, commit, change_id, title, body, []);
        // The merged-branch cleanup phase lists local stack bookmarks; default to
        // none so these tests exercise rebase/submit without cleanup side effects.
        outputs.push(fake_success(
            "jj",
            &["bookmark", "list", "-T", LOCAL_BOOKMARK_TEMPLATE],
            "",
        ));
    ...
    fn submit_one_change_outputs(
        ...
        let mut outputs = vec![
            ...
            fake_success("git", &["show", "-s", "--format=%T", commit], "tree"),
            fake_success("git", &["merge-base", commit, DEFAULT_TRUNK], root_parent),
            fake_success(
                "gh",
                &[
                    "repo",
    ...
    fn command_index(calls: &[String], command: &str) -> usize {
        calls
            .iter()
            .position(|call| call == command)
            .unwrap_or_else(|| panic!("missing `{command}` in calls: {calls:?}"))
    }

    fn test_pr_entry(pr_number: u64, head_branch: &str) -> PrCacheEntry {
        PrCacheEntry {
            pr_number,
            pr_node_id: format!("PR_node_{pr_number}"),
            head_branch: head_branch.to_owned(),
            base_branch: "main".to_owned(),
            base_ref: "main".to_owned(),
            head_repo_id: "repo-id".to_owned(),
            head_repo_node_id: "repo-node".to_owned(),
            head_repo_name: "owner/repo".to_owned(),
            base_repo_id: "repo-id".to_owned(),
            base_repo_node_id: "repo-node".to_owned(),
            base_repo_name: "owner/repo".to_owned(),
            head_sha: "headsha".to_owned(),
            base_sha: "basesha".to_owned(),
            author_login: "octocat".to_owned(),
            title: "changeone title".to_owned(),
            body: String::new(),
            created_at: "2026-06-03T12:34:56Z".to_owned(),
            stack_comment_id: Some("comment-123".to_owned()),
        }
    ...
    fn fake_jj_remote_trunk(commit: &str) -> (String, CommandOutput) {
        fake_success(
            "jj",
            &["log", "--no-graph", "-r", "main@origin", "-T", "commit_id"],
            commit,
    ...
    fn fake_pr_fetch(pr_number: u64, stdout: &str) -> (String, CommandOutput) {
        let endpoint = format!("repos/owner/repo/pulls/{pr_number}");
        fake_success("gh", &["api", endpoint.as_str(), "--jq", PR_API_JQ], stdout)
    }

    fn fake_local_bookmark(branch: &str, commit: &str) -> (String, CommandOutput) {
        fake_success(
            "jj",
            &["log", "--no-graph", "-r", branch, "-T", "commit_id"],
            commit,
        )
    }

    fn fake_local_stack_bookmarks<const N: usize>(
        change: &ResolvedChange,
        branches: [&str; N],
    ...
    fn fake_tracked_remote_bookmark(branch: &str) -> (String, CommandOutput) {
        fake_remote_bookmark_status(branch, true, false)
    }

    fn fake_frozen_bookmarks(stdout: &str) -> (String, CommandOutput) {
        fake_success(
            "jj",
            &[
                "bookmark",
    ...
    fn fake_remote_bookmark_status(
        branch: &str,
        tracked: bool,
        conflicted: bool,
    ) -> (String, CommandOutput) {
        let tracked = if tracked { "tracked" } else { "untracked" };
        let conflicted = if conflicted { "conflicted" } else { "ok" };
        fake_success(
            "jj",
            &[
                "bookmark",
            ...
            &format!("{DEFAULT_REMOTE}\t{tracked}\t{conflicted}\n"),
        )
    }

    fn fake_comment_list(pr_number: u64, stdout: &str) -> (String, CommandOutput) {
    ...
    fn fake_comment_list_failure(pr_number: u64, stderr: &str) -> (String, CommandOutput) {
        let endpoint = format!("repos/owner/repo/issues/{pr_number}/comments");
        fake_failure(
            "gh",
            &[
    ...
    fn fake_comment_create(pr_number: u64, body: &str, id: u64) -> (String, CommandOutput) {
        let endpoint = format!("repos/owner/repo/issues/{pr_number}/comments");
        let body_field = format!("body={body}");
        fake_success(
            "gh",
    ...
    fn fake_comment_update(comment_id: u64, body: &str, id: u64) -> (String, CommandOutput) {
    ...
    fn comment_line(id: u64, body: &str, user_login: &str, updated_at: &str) -> String {
        serde_json::json!({
            "id": id,
            "body": body,
    ...
    fn expected_stack_comment_body(
        rows: &[(&str, u64, &str, &str, &str)],
        current_change_id: &str,
    ) -> String {
        let stack = rows
            .iter()
            .map(|(change_id, _, _, _, title)| ResolvedChange {
                change_id: (*change_id).to_owned(),
        ...
        let entries = rows
            .iter()
            .map(|(change_id, pr_number, head_branch, base_branch, title)| {
                (
                    (*change_id).to_owned(),
                    PrCacheEntry {
                        pr_number: *pr_number,
                        ...
                        base_ref: (*base_branch).to_owned(),
                        head_sha: format!("{change_id}head"),
                        base_sha: format!("{change_id}base"),
                        title: (*title).to_owned(),
                        body: String::new(),
                        created_at: "2026-06-03T12:34:56Z".to_owned(),
                        stack_comment_id: None,
                        ..test_pr_entry(*pr_number, head_branch)
                    },
                )
            })
            .collect::<Vec<_>>();
        ...
        stack_comment_body(
            &AppContext::from_owned(test_github(), stack),
            &entries,
            current_change_id,
            DEFAULT_TRUNK,
        )
    ...
    fn pr_json(
        ...
        pr_json_with_details(
            ...
            head_sha,
            base_sha,
            &format!("{head_branch} title"),
            "",
        )
    }

    fn pr_json_with_details(
        number: u64,
        state: &str,
        head_branch: &str,
        base_branch: &str,
    ...
    fn fake_merge_pr_view(pr_number: u64, stdout: &str) -> (String, CommandOutput) {
    ...
    fn unique_test_dir(name: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        let path = env::temp_dir().join(format!("stack-{name}-{}-{nanos}", process::id()));
    ...
    fn stack_record(
        ...
        [
            serde_json::to_string(change_id)
                .unwrap_or_else(|error| panic!("serialize change id: {error}")),
            serde_json::to_string(commit_id)
                .unwrap_or_else(|error| panic!("serialize commit id: {error}")),
            serde_json::to_string(parent_ids)
                .unwrap_or_else(|error| panic!("serialize parent ids: {error}")),
            serde_json::to_string(title).unwrap_or_else(|error| panic!("serialize title: {error}")),
            serde_json::to_string(description)
                .unwrap_or_else(|error| panic!("serialize description: {error}")),
            serde_json::to_string(&empty)
                .unwrap_or_else(|error| panic!("serialize empty: {error}")),
            serde_json::to_string(&conflict)
                .unwrap_or_else(|error| panic!("serialize conflict: {error}")),
        ]
